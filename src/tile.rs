...
impl Tile {
	...
	pub fn parse_run<N>(s: &str) -> Result<(ArrayVec<Self, N>, Option<HandMeldType>), ()> where N: ArrayLength {
	...
	pub const fn indicates_dora(self, game_type: GameType) -> Self {
		...
		let add02 = if (0b0110111_011111111_011111111_011111111_u64 & offset) != 0 { 2 } else { 0 };
		let sub16 = if (0b0000000_100000000_100000000_100000000_u64 & offset) != 0 { 16 } else { 0 };
		let sub06 = if this == t!(N) as u8 { 6 } else { 0 };
		let sub04 = if this == t!(R) as u8 { 4 } else { 0 };
		let add14 = if (this & !0b1) == t!(1m) as u8 && matches!(game_type, GameType::Sanma) { 14 } else { 0 };

		let result = (this & !0b1) + add02 - sub16 - sub06 - sub04 + add14;
...
impl Ord for Number {
	fn cmp(&self, other: &Self) -> core::cmp::Ordering {
		let this = (*self as u8) & 0x1E;
		let other = (*other as u8) & 0x1E;
		this.cmp(&other)
	}
}
