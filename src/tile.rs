...
impl Tile {
	...
	pub fn indicates_dora(self, game_type: GameType) -> Self {
		...
		let (yonma, sanma) = DORAS[(self as usize) >> 1];
		match game_type {
...
impl Ord for Tile {
	fn cmp(&self, other: &Self) -> core::cmp::Ordering {
		let this = (*self as u8) & 0x7E;
		let other = (*other as u8) & 0x7E;
		this.cmp(&other)
	}
}
...
impl NumberTile {
	...
	pub(crate) const fn previous_in_sequence(self) -> Option<Self> {
		...
		let n = self as u8;
		let prev = (n & 0x3E).wrapping_sub(2);
		let n = if n >= MAX { n - MAX } else { n };
		let n = if n >= MAX { n - MAX } else { n };
		if n >= 0x02 {
	...
	pub(crate) fn is_next_in_sequence(self, previous: Self) -> bool {
		previous.next_in_sequence() == Some(self)
	}
}
...
mod tests {
	...
	#[test]
	fn number_tile_as_ref_and_from() {
