...
impl NumberTile {
	...
	pub(crate) const fn shun_rest(self) -> Option<(Self, Self)> {
		...
		//
		// Tested exhaustively in the `number_tile_shun_rest` test.

		if 0b110000000_110000000_110000000_u32 & (1_u32 << (self as u8 >> 1)) == 0 {
			let n = self as u8;
			let next = (n & !0b1) + 2;
...
impl TryFrom<Tile> for NumberTile {
	...
	fn try_from(t: Tile) -> Result<Self, Self::Error> {
		if t >= t!(1m) && t <= t!(9s) {
			...
			let t = unsafe { core::mem::transmute::<Tile, Self>(t) };
			Ok(t)
		}
		else {
			Err(())
...
mod tests {
	...
	fn number_tile_shun_rest() {
		assert_eq!(tn!(1m).shun_rest(), Some((tn!(2m), tn!(3m))));
		assert_eq!(tn!(2m).shun_rest(), Some((tn!(3m), tn!(4m))));
		assert_eq!(tn!(3m).shun_rest(), Some((tn!(4m), tn!(5m))));
		assert_eq!(tn!(4m).shun_rest(), Some((tn!(5m), tn!(6m))));
		assert_eq!(tn!(5m).shun_rest(), Some((tn!(6m), tn!(7m))));
		assert_eq!(tn!(0m).shun_rest(), Some((tn!(6m), tn!(7m))));
		assert_eq!(tn!(6m).shun_rest(), Some((tn!(7m), tn!(8m))));
		assert_eq!(tn!(7m).shun_rest(), Some((tn!(8m), tn!(9m))));
		assert_eq!(tn!(8m).shun_rest(), None);
		assert_eq!(tn!(9m).shun_rest(), None);

		assert_eq!(tn!(1p).shun_rest(), Some((tn!(2p), tn!(3p))));
		assert_eq!(tn!(2p).shun_rest(), Some((tn!(3p), tn!(4p))));
		assert_eq!(tn!(3p).shun_rest(), Some((tn!(4p), tn!(5p))));
		assert_eq!(tn!(4p).shun_rest(), Some((tn!(5p), tn!(6p))));
		assert_eq!(tn!(5p).shun_rest(), Some((tn!(6p), tn!(7p))));
		assert_eq!(tn!(0p).shun_rest(), Some((tn!(6p), tn!(7p))));
		assert_eq!(tn!(6p).shun_rest(), Some((tn!(7p), tn!(8p))));
		assert_eq!(tn!(7p).shun_rest(), Some((tn!(8p), tn!(9p))));
		assert_eq!(tn!(8p).shun_rest(), None);
		assert_eq!(tn!(9p).shun_rest(), None);

		assert_eq!(tn!(1s).shun_rest(), Some((tn!(2s), tn!(3s))));
		assert_eq!(tn!(2s).shun_rest(), Some((tn!(3s), tn!(4s))));
		assert_eq!(tn!(3s).shun_rest(), Some((tn!(4s), tn!(5s))));
		assert_eq!(tn!(4s).shun_rest(), Some((tn!(5s), tn!(6s))));
		assert_eq!(tn!(5s).shun_rest(), Some((tn!(6s), tn!(7s))));
		assert_eq!(tn!(0s).shun_rest(), Some((tn!(6s), tn!(7s))));
		assert_eq!(tn!(6s).shun_rest(), Some((tn!(7s), tn!(8s))));
		assert_eq!(tn!(7s).shun_rest(), Some((tn!(8s), tn!(9s))));
		assert_eq!(tn!(8s).shun_rest(), None);
		assert_eq!(tn!(9s).shun_rest(), None);
	}
