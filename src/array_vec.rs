...
impl<T, const CAPACITY: usize> Clone for ArrayVecIntoIter<T, CAPACITY> where T: Clone {
	fn clone(&self) -> Self {
		...
		let mut inner = [const { core::mem::MaybeUninit::uninit() }; CAPACITY];
...
impl<T, const CAPACITY: usize> core::fmt::Debug for ArrayVecIntoIter<T, CAPACITY> where T: core::fmt::Debug {
	fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
		unsafe { core::hint::assert_unchecked(self.range.start <= self.range.end) };
		unsafe { core::hint::assert_unchecked(self.range.end <= self.inner.len()) };
