...
impl<T, N, CAPACITY> TryFrom<GenericArray<T, N>> for ArrayVec<T, CAPACITY> where N: ArrayLength, CAPACITY: ArrayLength {
	...
	fn try_from(arr: GenericArray<T, N>) -> Result<Self, Self::Error> {
		match N::USIZE.cmp(&CAPACITY::USIZE) {
			core::cmp::Ordering::Less => Ok(arr.into_iter().collect()),
			core::cmp::Ordering::Equal => {
				let inner = unsafe { core::mem::transmute_copy::<GenericArray<T, N>, GenericArray<core::mem::MaybeUninit<T>, CAPACITY>>(&arr) };
				core::mem::forget(arr);
				Ok(Self { inner, len: N::USIZE })
			},
			core::cmp::Ordering::Greater => Err(arr),
		}
	}
}
