...
use crate::{
	...
	Tile, Tile27Set, Tile37SuitedMultiSet, TsumoOrRon,
...
impl LookupInner {
	fn new(ts: &Tile37SuitedMultiSetInner) -> Self {
		...
		let ji = { let (counts, total) = ts.ji(); lookup_honors(counts, total) };
		let sou = { let (counts, total) = ts.sou(); lookup_numbers(counts, total) };
		let pin = { let (counts, total) = ts.pin(); lookup_numbers(counts, total) };
		let man = { let (counts, total) = ts.man(); lookup_numbers(counts, total) };
