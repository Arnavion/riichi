	...
	fn next(&mut self) -> Option<Self::Item> {
		loop {
			let Some((ms, md, pair)) = self.current.next() else {
				...
				let current = extract_fourth_meld(ms, pair, self.new_tile, self.tsumo_or_ron);
...
pub(crate) struct LookupForTenhou {
	current_a: core::array::IntoIter<([ScorableHandMeld; 3], ScorableHandFourthMeld, ScorableHandPair), 4>,
...
impl Iterator for LookupForTenhou {
	type Item = ([ScorableHandMeld; 3], ScorableHandFourthMeld, ScorableHandPair);
	...
	fn next(&mut self) -> Option<Self::Item> {
		loop {
			let Some((ms, md, pair)) = self.current_a.next() else {
				...
				let current = extract_fourth_meld(*ms, *pair, new_tile, TsumoOrRon::Tsumo);
				self.current_a = current.into_iter();
				continue;
			};
			break Some((ms, md, pair));
		}
...
fn extract_fourth_meld<const NM: usize>(
	...
	new_tile: Tile,
	tsumo_or_ron: TsumoOrRon,
) -> core::array::IntoIter<([ScorableHandMeld; NM], ScorableHandFourthMeld, ScorableHandPair), { (NM + 2).min(4) }>
{
...
mod tests {
	...
	fn to_meld() {
		for ma in melds_last() {
			...
			let actual: std::vec::Vec<_> = LookupForNewTile::new(Lookup::<1>::new(&Tile37CountedMultiSet::new(&ts).unwrap().insert(new_tile).unwrap()), new_tile, tsumo_or_ron).collect();
	...
	fn to_melds_2() {
		for ma in melds() {
			...
			for mb in melds().into_iter().map(ScorableHandFourthMeld::tanki).chain(melds_last()) {
				...
				let actual: std::vec::Vec<_> = LookupForNewTile::new(Lookup::<2>::new(&Tile37CountedMultiSet::new(&ts).unwrap().insert(new_tile).unwrap()), new_tile, tsumo_or_ron).collect();
				assert!(
		...
		for ma in melds() {
			...
			for mb in melds() {
				...
				for mc in melds().into_iter().map(ScorableHandFourthMeld::tanki).chain(melds_last()) {
					...
					let mut actual: std::vec::Vec<_> = LookupForNewTile::new(Lookup::<3>::new(&Tile37CountedMultiSet::new(&ts).unwrap().insert(new_tile).unwrap()), new_tile, tsumo_or_ron).collect();
			...
			for mb in melds() {
				...
				for mc in melds() {
					...
					for md in melds().into_iter().map(ScorableHandFourthMeld::tanki).chain(melds_last()) {
						...
						let mut actual: std::vec::Vec<_> = LookupForNewTile::new(Lookup::<4>::new(&Tile37CountedMultiSet::new(&ts).unwrap().insert(new_tile).unwrap()), new_tile, tsumo_or_ron).collect();
						for (ms, ..) in &mut actual { ms.sort_unstable(); }
						assert!(
							expected.iter().any(|expected| actual.contains(expected)),
