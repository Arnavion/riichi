...
impl LookupInner {
	fn new(ts: &Tile37MultiSet) -> Self {
		...
		fn lookup_honors((key, len): (u32, u8)) -> Option<&'static (u32, Option<Honor>, Meld<Honor>)> {
			let map = [
				honors::ZEROS,
				&[],
				honors::TWOS,
		...
		if
			let Some(ji @ (_, ji_pair, _)) = lookup_honors(ts.ji()) &&
			...
			usize::from(ji_pair.is_some()) + usize::from(sou_pair.is_some()) + usize::from(pin_pair.is_some()) + usize::from(man_pair.is_some()) == 1
		{
			result.ji = Some(ji);
			result.sou = sou;
			result.pin = pin;
	...
	unsafe fn next_to(&mut self, melds: &mut [core::mem::MaybeUninit<ScorableHandMeld>]) -> Option<ScorableHandPair> {
		...
		let (man_pair, man_melds) = self.man[0];
			...
			let Some((ms, md, pair)) = self.current.next() else {
				...
				for (i, &md) in ms.iter().enumerate() {
					if let Some(md) = new_tile_in_meld(md, self.new_tile, self.tsumo_or_ron) {
						...
						let result = current.push((ms, md, pair));
						unsafe { result.unwrap_unchecked(); }
					}
				}
