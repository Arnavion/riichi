...
impl<NM> ExactSizeIterator for Lookup<NM> where Self: Iterator {
	fn len(&self) -> usize {
		self.0.len()
	}
}
...
where
	...
	GenericArray<ScorableHandMeld, Sum<NM, U1>>: Copy + Remove<ScorableHandMeld, Sum<NM, U1>, Output = GenericArray<ScorableHandMeld, NM>> + Shorten<ScorableHandMeld, Shorter = GenericArray<ScorableHandMeld, NM>>,
{
