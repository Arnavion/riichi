...
#[derive(Debug)]
#[derive_const(Clone)]
pub(crate) struct Keys<const NT: usize> {
	man: u32,
	i_man: u8,
	pin: u32,
	i_pin: u8,
...
impl<const NT: usize> Keys<NT> {
	pub(crate) fn push(mut self, t: Tile) -> Keys<{ NT + 1 }> {
		self.push_inner(t);
		Keys {
			man: self.man,
			i_man: self.i_man,
	...
	pub(crate) fn push_all<const N: usize>(mut self, ts: [Tile; N]) -> Keys<{ NT + N }> {
		for t in ts {
			self.push_inner(t);
		}
		Keys {
...
#[derive(Clone, Debug)]
pub(crate) struct Lookup<const NM: usize> {
...
impl<const NM: usize> Lookup<NM> {
	...
	pub(crate) fn new(keys: Keys<{ NM * 3 + 2 }>) -> Self {
		...
		let man = lookup_numbers(keys.man, keys.i_man);
		let pin = lookup_numbers(keys.pin, keys.i_pin);
		let sou = lookup_numbers(keys.sou, keys.i_sou);
		let ji = lookup_honors(keys.ji, keys.i_ji);
...
impl<const NM: usize> Iterator for Lookup<NM> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		let mut i_man = usize::from(self.i_man);
		...
		unsafe { core::hint::assert_unchecked(i_man < self.man.len()); }
		let (man_melds, man_pair) = self.man[i_man];
		...
		for ms in [man_melds.with_suit(NumberSuit::Man), pin_melds.with_suit(NumberSuit::Pin), sou_melds.with_suit(NumberSuit::Sou), ji_melds.into()] {
			match ms {
				Meld::M0 => (),

				Meld::M1(m1) => {
					unsafe { core::hint::assert_unchecked(NM >= 1); }
					unsafe { melds.push_unchecked(m1); }
				},

				Meld::M2(m1, m2) => {
					unsafe { core::hint::assert_unchecked(NM >= 2); }
					unsafe { melds.push_unchecked(m1); }
					unsafe { melds.push_unchecked(m2); }
				},

				Meld::M3(m1, m2, m3) => {
					unsafe { core::hint::assert_unchecked(NM >= 3); }
					unsafe { melds.push_unchecked(m1); }
					unsafe { melds.push_unchecked(m2); }
				...
				Meld::M4(m1, m2, m3, m4) => {
					unsafe { core::hint::assert_unchecked(NM >= 4); }
					unsafe { melds.push_unchecked(m1); }
					unsafe { melds.push_unchecked(m2); }
					unsafe { melds.push_unchecked(m3); }
		...
		// If one or more of the tiles did not found a valid meld, the corresponding slice would've been empty and we would've returned when `self.i_x >= self.x.len()`.
		let melds = unsafe { melds.unwrap_unchecked() };

		#[expect(clippy::cast_possible_truncation)]
		{
			self.i_man = i_man as u8;
	...
	fn size_hint(&self) -> (usize, Option<usize>) {
		...
		let processed =
			usize::from(self.i_man) * self.pin.len() * self.sou.len() * usize::from(self.ji.is_some()) +
		...
		loop {
			let Some((ms, md, pair)) = self.current.next() else {
				...
				if pair.0.eq_ignore_red(&self.new_tile) {
					let md = ms[0];
					let ms = unsafe { except::<_, { NM + 1 }, 1>(&ms, [0]) };
...
mod tests {
	...
	fn to_meld() {
		for ma in melds_last() {
			...
			let actual: std::vec::Vec<_> = LookupForNewTile::<0>::new(Lookup::<1>::new(Keys::default().push_all(ts).push(new_tile)), new_tile, tsumo_or_ron).collect();
			assert_eq!(actual, [expected], "{ma:?} did not meld into {expected:?}, only into {actual:?}");
		}

		// 124 -> X
		assert!(Lookup::<1>::new(Keys::default().push_all([t!(1s), t!(2s), t!(4s), t!(1p), t!(1p)])).next().is_none());
	}
	...
	fn to_melds_2() {
		for ma in melds() {
			...
			for mb in melds().into_iter().map(ScorableHandFourthMeld::Tanki).chain(melds_last()) {
				...
				let actual: std::vec::Vec<_> = LookupForNewTile::<1>::new(Lookup::<2>::new(Keys::default().push_all(ts).push(new_tile)), new_tile, tsumo_or_ron).collect();
				assert!(
		...
		for ma in melds() {
			...
			for mb in melds() {
				...
				for mc in melds().into_iter().map(ScorableHandFourthMeld::Tanki).chain(melds_last()) {
					...
					let mut actual: std::vec::Vec<_> = LookupForNewTile::<2>::new(Lookup::<3>::new(Keys::default().push_all(ts).push(new_tile)), new_tile, tsumo_or_ron).collect();
			...
			for mb in melds() {
				...
				for mc in melds() {
					...
					for md in melds().into_iter().map(ScorableHandFourthMeld::Tanki).chain(melds_last()) {
						...
						let mut actual: std::vec::Vec<_> = LookupForNewTile::<3>::new(Lookup::<4>::new(Keys::default().push_all(ts).push(new_tile)), new_tile, tsumo_or_ron).collect();
						for (ms, ..) in &mut actual { ms.sort_unstable(); }
						assert!(
							expected.iter().any(|expected| actual.contains(expected)),
