...
use crate::{
	...
	Tile, Tile27Set, Tile37Set, TsumoOrRon,
};
...
impl Tile37SuitedMultiSetInner {
	...
	const fn tenpai(&self) -> Tile37Set {
		const fn needs_tile(total: u8) -> bool {
			total < 14 && (0b10110110110110 >> total) & 0b1 == 0b1
		}
		...
		if needs_tile(self.totals[0]) {
			present |= counts_to_present_numbers(self.counts[0]);
		}
		if needs_tile(self.totals[1]) {
			present |= counts_to_present_numbers(self.counts[1]) << 10;
		}
		if needs_tile(self.totals[2]) {
			present |= counts_to_present_numbers(self.counts[2]) << 20;
		}
...
impl LookupInner {
	fn new(ts: &Tile37SuitedMultiSetInner) -> Self {
		...
		let ji = lookup_honors(ts.counts[3], ts.totals[3]);
		let sou = lookup_numbers(ts.counts[2], ts.totals[2]);
		let pin = lookup_numbers(ts.counts[1], ts.totals[1]);
		let man = lookup_numbers(ts.counts[0], ts.totals[0]);
