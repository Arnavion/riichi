...
fn main_inner(stdout: &mut impl std::io::Write, arg0: &str, mut args: impl Iterator<Item = String>) -> Result<(), ()> {
	...
	let other_waits = if let Some(winning_tile) = args.next() {
		...
		let other_waits: Tile37Set = hand.clone().tenpai().filter(|&t| t != winning_tile).collect();
		...
		let waits: Tile37Set = hand.clone().tenpai().collect();
		if waits.is_empty() {
			_ = writeln!(stdout, "No yaku");
			return Ok(());
		}
...
Usage: {arg0} [GAME_TYPE] <ROUND_WIND> <SEAT_WIND> <DEAD_WALL> <HAND> [RIICHI] <WINNING_TILE_FROM> [WINNING_TILE]
    ...
    HAND: Specified in MPSZ format, extended to support notating minjuns / minkous / ankans / minkans. Ref: https://note.com/yuarasino/n/n1ba95bf3b618
    ...
    WINNING_TILE_FROM: One of haitei, houtei-shimocha, houtei-toimen, houtei-kamicha, rinshan, chankan-shimocha, chankan-toimen, chankan-kamicha, tenhou, chiihou, tsumo, ron-shimocha, ron-toimen, ron-kamicha.
