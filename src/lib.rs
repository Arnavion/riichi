...
macro_rules! make_scorable_hand {
	...
	(@meldr4 { ankan $t1:tt $t2:tt $t3:tt $t4:tt }) => {
		$crate::ScorableHandFourthMeld::from($crate::ScorableHandMeld::ankan($crate::t!($t1), $crate::t!($t2), $crate::t!($t3), $crate::t!($t4)).unwrap())
	};
	...
	(@meldr4 { minkan $t1:tt $t2:tt $t3:tt $t4:tt }) => {
		$crate::ScorableHandFourthMeld::from($crate::ScorableHandMeld::minkan($crate::t!($t1), $crate::t!($t2), $crate::t!($t3), $crate::t!($t4)).unwrap())
	};
	...
	(@meldr4 { ankou $t1:tt $t2:tt $t3:tt }) => {
		$crate::ScorableHandFourthMeld::from($crate::ScorableHandMeld::ankou($crate::t!($t1), $crate::t!($t2), $crate::t!($t3)).unwrap())
	};
	...
	(@meldr4 { minkou $t1:tt $t2:tt $t3:tt }) => {
		$crate::ScorableHandFourthMeld::from($crate::ScorableHandMeld::minkou($crate::t!($t1), $crate::t!($t2), $crate::t!($t3)).unwrap())
	};
	...
	(@meldr4 { anjun $t1:tt $t2:tt $t3:tt }) => {{
		...
		$crate::SortingNetwork::sort(&mut tiles);
		let t1 = tiles[0].try_into().unwrap(); // Guaranteed to be valid because it was run through `tsl!()` above.
		$crate::ScorableHandFourthMeld::from($crate::ScorableHandMeld::anjun(t1, tiles[1], tiles[2]).unwrap())
	}};
	...
	(@meldr4 { minjun $t1:tt $t2:tt $t3:tt }) => {{
		...
		$crate::SortingNetwork::sort(&mut tiles);
		let t1 = tiles[0].try_into().unwrap(); // Guaranteed to be valid because it was run through `tsl!()` above.
		$crate::ScorableHandFourthMeld::from($crate::ScorableHandMeld::minjun(t1, tiles[1], tiles[2]).unwrap())
	}};
...
mod tile_set;
pub use tile_set::{
	Tile27Set, Tile27SetIntoIter, Tile34Set, Tile34SetIntoIter,
};
...
unsafe fn except<T, N, DN>(
	ts: &generic_array::GenericArray<T, N>,
	i_discard: generic_array::GenericArray<usize, DN>,
) -> generic_array::GenericArray<T, generic_array::typenum::Diff<N, DN>>
where
	T: Clone,
	N: generic_array::ArrayLength + core::ops::Sub<DN, Output: generic_array::ArrayLength>,
	DN: generic_array::ArrayLength,
{
	let mut result = generic_array::GenericArray::uninit();
	...
	unsafe { generic_array::GenericArray::assume_init(result) }
}
