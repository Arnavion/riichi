...
//! let mut hands = hand.to_scorable_hands(t!(9s), winning_tile_from.into());
...
//! assert_eq!(score, Score { fu: 50, han: 2, yakuman: 1 });
//!
//! let is_dealer = seat_wind == tw!(E);
//! let score = score.score(is_dealer, winning_tile_from.into());
//! assert_eq!(score, Points::FromEveryone { dealer: 16000, non_dealer: 8000 });
//!
...
mod hand;
pub use hand::{
	Hand, HandMeld, HandMinjuns, HandMinkous, HandStable, HandTentative,
	Minjuns, Minkous,
};

mod scorable_hand;
pub use scorable_hand::{
	ScorableHand, ScorableHandFourthMeld, ScorableHandIntoIter, ScorableHandMeld, ScorableHandMeldIntoIter, ScorableHandPair,
};
...
pub use tile_multi_set::{
	TileMultiSet, Tile27MultiSet, Tile34MultiSet, Tile37MultiSet,
