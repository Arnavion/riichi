...
///         ],
///         ScorableHandFourthMeld::RyanmenLow { tiles: tn![2s, 3s, 4s], tsumo_or_ron: TsumoOrRon::Ron },
///     ),
...
/// # #![deny(unused)]
/// #
/// # use riichi::{ScorableHand, ScorableHandPair, make_scorable_hand, t};
/// #
...
macro_rules! make_scorable_hand {
	...
	(@meldr4 { anjun $t1:tt $t2:tt $t3:tt ryanmen_low }) => {{
		let mut tiles = $crate::tn![$t1, $t2, $t3];
		$crate::SortingNetwork::sort_three(&mut tiles);
		$crate::ScorableHandFourthMeld::RyanmenLow { tiles, tsumo_or_ron: $crate::TsumoOrRon::Tsumo }
	}};

	(@meldr4 { anjun $t1:tt $t2:tt $t3:tt ryanmen_high }) => {{
		let mut tiles = $crate::tn![$t1, $t2, $t3];
		$crate::SortingNetwork::sort_three(&mut tiles);
		$crate::ScorableHandFourthMeld::RyanmenHigh { tiles, tsumo_or_ron: $crate::TsumoOrRon::Tsumo }
	}};
	...
	(@meldr4 { minjun $t1:tt $t2:tt $t3:tt ryanmen_low }) => {{
		let mut tiles = $crate::tn![$t1, $t2, $t3];
		$crate::SortingNetwork::sort_three(&mut tiles);
		$crate::ScorableHandFourthMeld::RyanmenLow { tiles, tsumo_or_ron: $crate::TsumoOrRon::Ron }
	}};

	(@meldr4 { minjun $t1:tt $t2:tt $t3:tt ryanmen_high }) => {{
		let mut tiles = $crate::tn![$t1, $t2, $t3];
		$crate::SortingNetwork::sort_three(&mut tiles);
		$crate::ScorableHandFourthMeld::RyanmenHigh { tiles, tsumo_or_ron: $crate::TsumoOrRon::Ron }
	}};
...
pub use scorable_hand::{
	ScorableHand, ScorableHandFourthMeld, ScorableHandMeld, ScorableHandMeldIntoIter, ScorableHandPair,
};
...
pub trait SortingNetwork: core::cmp::PartialOrd + Sized {
	fn sort_three(arr: &mut [Self; 3]) {
		// `arr.sort_unstable()` generates excessively verbose code as of Rust 1.91 and contemporary nightly,
