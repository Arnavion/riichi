...
///     melds: (
///         [
///             ScorableHandMeld::anjun(tsl!(2s), tn!(3s), tn!(4s)),
///             ScorableHandMeld::ankan(t!(E), t!(E), t!(E), t!(E)),
///             ScorableHandMeld::ankou(t!(Wh), t!(Wh), t!(Wh)),
///         ],
///         ScorableHandFourthMeld::ryanmen_low(tsl!(2s), tn!(3s), tn!(4s), TsumoOrRon::Ron),
///     ),
///     pair: ScorableHandPair::new(t!(8s), t!(8s)),
/// }));
/// ```
///
/// ```rust
...
///     ScorableHandPair::new(t!(1m), t!(1m)),
///     ScorableHandPair::new(t!(3m), t!(3m)),
///     ScorableHandPair::new(t!(4m), t!(4m)),
///     ScorableHandPair::new(t!(5p), t!(5p)),
///     ScorableHandPair::new(t!(2s), t!(2s)),
///     ScorableHandPair::new(t!(W), t!(W)),
///     ScorableHandPair::new(t!(Wh), t!(Wh)),
/// ])));
/// ```
///
/// ```rust
...
macro_rules! make_scorable_hand {
	(@meld { ankan $t1:tt $t2:tt $t3:tt $t4:tt }) => {
		$crate::ScorableHandMeld::ankan($crate::t!($t1), $crate::t!($t2), $crate::t!($t3), $crate::t!($t4))
	};

	(@meldr4 { ankan $t1:tt $t2:tt $t3:tt $t4:tt }) => {
		$crate::ScorableHandFourthMeld::Tanki($crate::ScorableHandMeld::ankan($crate::t!($t1), $crate::t!($t2), $crate::t!($t3), $crate::t!($t4)))
	};

	(@meld { minkan $t1:tt $t2:tt $t3:tt $t4:tt }) => {
		$crate::ScorableHandMeld::minkan($crate::t!($t1), $crate::t!($t2), $crate::t!($t3), $crate::t!($t4))
	};

	(@meldr4 { minkan $t1:tt $t2:tt $t3:tt $t4:tt }) => {
		$crate::ScorableHandFourthMeld::Tanki($crate::ScorableHandMeld::minkan($crate::t!($t1), $crate::t!($t2), $crate::t!($t3), $crate::t!($t4)))
	};

	(@meld { ankou $t1:tt $t2:tt $t3:tt }) => {
		$crate::ScorableHandMeld::ankou($crate::t!($t1), $crate::t!($t2), $crate::t!($t3))
	};

	(@meldr4 { ankou $t1:tt $t2:tt $t3:tt }) => {
		$crate::ScorableHandFourthMeld::Tanki($crate::ScorableHandMeld::ankou($crate::t!($t1), $crate::t!($t2), $crate::t!($t3)))
	};

	(@meldr4 { ankou $t1:tt $t2:tt $t3:tt shanpon }) => {
		$crate::ScorableHandFourthMeld::shanpon($crate::t!($t1), $crate::t!($t2), $crate::t!($t3), $crate::TsumoOrRon::Tsumo)
	};

	(@meld { minkou $t1:tt $t2:tt $t3:tt }) => {
		$crate::ScorableHandMeld::minkou($crate::t!($t1), $crate::t!($t2), $crate::t!($t3))
	};

	(@meldr4 { minkou $t1:tt $t2:tt $t3:tt }) => {
		$crate::ScorableHandFourthMeld::Tanki($crate::ScorableHandMeld::minkou($crate::t!($t1), $crate::t!($t2), $crate::t!($t3)))
	};

	(@meldr4 { minkou $t1:tt $t2:tt $t3:tt shanpon }) => {
		$crate::ScorableHandFourthMeld::shanpon($crate::t!($t1), $crate::t!($t2), $crate::t!($t3), $crate::TsumoOrRon::Ron)
	};

	(@meld { anjun $t1:tt $t2:tt $t3:tt }) => {{
		let mut tiles = [$crate::NumberTile::from($crate::tsl!($t1)), $crate::tn!($t2), $crate::tn!($t3)];
		$crate::SortingNetwork::sort_three(&mut tiles);
		let t1 = tiles[0].try_into().unwrap(); // Guaranteed to be valid because it was run through `tsl!()` above.
		$crate::ScorableHandMeld::anjun(t1, tiles[1], tiles[2])
	}};
	...
	(@meldr4 { anjun $t1:tt $t2:tt $t3:tt }) => {{
		...
		$crate::SortingNetwork::sort_three(&mut tiles);
		let t1 = tiles[0].try_into().unwrap(); // Guaranteed to be valid because it was run through `tsl!()` above.
		$crate::ScorableHandFourthMeld::Tanki($crate::ScorableHandMeld::anjun(t1, tiles[1], tiles[2]))
	}};
	...
	(@meldr4 { anjun $t1:tt $t2:tt $t3:tt kanchan }) => {{
		...
		$crate::SortingNetwork::sort_three(&mut tiles);
		let t1 = tiles[0].try_into().unwrap(); // Guaranteed to be valid because it was run through `tsl!()` above.
		$crate::ScorableHandFourthMeld::kanchan(t1, tiles[1], tiles[2], $crate::TsumoOrRon::Tsumo)
	}};
	...
	(@meldr4 { anjun $t1:tt $t2:tt $t3:tt penchan }) => {{
		...
		$crate::SortingNetwork::sort_three(&mut tiles);
		let t1 = tiles[0].try_into().unwrap(); // Guaranteed to be valid because it was run through `tsl!()` above.
		$crate::ScorableHandFourthMeld::penchan(t1, tiles[1], tiles[2], $crate::TsumoOrRon::Tsumo)
	}};
	...
	(@meldr4 { anjun $t1:tt $t2:tt $t3:tt ryanmen_low }) => {{
		...
		$crate::SortingNetwork::sort_three(&mut tiles);
		let t1 = tiles[0].try_into().unwrap(); // Guaranteed to be valid because it was run through `tsl!()` above.
		$crate::ScorableHandFourthMeld::ryanmen_low(t1, tiles[1], tiles[2], $crate::TsumoOrRon::Tsumo)
	}};
	...
	(@meldr4 { anjun $t1:tt $t2:tt $t3:tt ryanmen_high }) => {{
		...
		$crate::SortingNetwork::sort_three(&mut tiles);
		let t1 = tiles[0].try_into().unwrap(); // Guaranteed to be valid because it was run through `tsl!()` above.
		$crate::ScorableHandFourthMeld::ryanmen_high(t1, tiles[1], tiles[2], $crate::TsumoOrRon::Tsumo)
	}};
	...
	(@meld { minjun $t1:tt $t2:tt $t3:tt }) => {{
		...
		$crate::SortingNetwork::sort_three(&mut tiles);
		let t1 = tiles[0].try_into().unwrap(); // Guaranteed to be valid because it was run through `tsl!()` above.
		$crate::ScorableHandMeld::minjun(t1, tiles[1], tiles[2])
	}};
	...
	(@meldr4 { minjun $t1:tt $t2:tt $t3:tt }) => {{
		...
		$crate::SortingNetwork::sort_three(&mut tiles);
		let t1 = tiles[0].try_into().unwrap(); // Guaranteed to be valid because it was run through `tsl!()` above.
		$crate::ScorableHandFourthMeld::Tanki($crate::ScorableHandMeld::minjun(t1, tiles[1], tiles[2]))
	}};
	...
	(@meldr4 { minjun $t1:tt $t2:tt $t3:tt kanchan }) => {{
		...
		$crate::SortingNetwork::sort_three(&mut tiles);
		let t1 = tiles[0].try_into().unwrap(); // Guaranteed to be valid because it was run through `tsl!()` above.
		$crate::ScorableHandFourthMeld::kanchan(t1, tiles[1], tiles[2], $crate::TsumoOrRon::Ron)
	}};
	...
	(@meldr4 { minjun $t1:tt $t2:tt $t3:tt penchan }) => {{
		...
		$crate::SortingNetwork::sort_three(&mut tiles);
		let t1 = tiles[0].try_into().unwrap(); // Guaranteed to be valid because it was run through `tsl!()` above.
		$crate::ScorableHandFourthMeld::penchan(t1, tiles[1], tiles[2], $crate::TsumoOrRon::Ron)
	}};
	...
	(@meldr4 { minjun $t1:tt $t2:tt $t3:tt ryanmen_low }) => {{
		...
		$crate::SortingNetwork::sort_three(&mut tiles);
		let t1 = tiles[0].try_into().unwrap(); // Guaranteed to be valid because it was run through `tsl!()` above.
		$crate::ScorableHandFourthMeld::ryanmen_low(t1, tiles[1], tiles[2], $crate::TsumoOrRon::Ron)
	}};
	...
	(@meldr4 { minjun $t1:tt $t2:tt $t3:tt ryanmen_high }) => {{
		...
		$crate::SortingNetwork::sort_three(&mut tiles);
		let t1 = tiles[0].try_into().unwrap(); // Guaranteed to be valid because it was run through `tsl!()` above.
		$crate::ScorableHandFourthMeld::ryanmen_high(t1, tiles[1], tiles[2], $crate::TsumoOrRon::Ron)
	}};
	...
	($m1:tt $m2:tt $m3:tt $m4:tt { $p1:tt $p2:tt }) => {{
		...
		$crate::ScorableHand::Regular($crate::ScorableHandRegular::new(
			[m1, m2, m3],
			m4,
			$crate::ScorableHandPair::new($crate::t!($p1), $crate::t!($p2)),
		))
	}};
	...
	({ $p1:tt $p2:tt } { $p3:tt $p4:tt } { $p5:tt $p6:tt } { $p7:tt $p8:tt } { $p9:tt $p10:tt } { $p11:tt $p12:tt } { $p13:tt $p14:tt }) => {{
		let mut pairs = [
			$crate::ScorableHandPair::new($crate::t!($p1), $crate::t!($p2)),
			$crate::ScorableHandPair::new($crate::t!($p3), $crate::t!($p4)),
			$crate::ScorableHandPair::new($crate::t!($p5), $crate::t!($p6)),
			$crate::ScorableHandPair::new($crate::t!($p7), $crate::t!($p8)),
			$crate::ScorableHandPair::new($crate::t!($p9), $crate::t!($p10)),
			$crate::ScorableHandPair::new($crate::t!($p11), $crate::t!($p12)),
			$crate::ScorableHandPair::new($crate::t!($p13), $crate::t!($p14)),
		];
		pairs.sort_unstable();
...
macro_rules! assert_size_of {
	($ty:ty, $size:expr) => { const _: () = [()][(core::mem::size_of::<$ty>() != $size) as usize]; };
}
...
pub use hand::{
	...
	Hand4ScorableHands, HandScorableHands,
	Minjuns, Minkous,
	Shouminkans,
};
...
pub use scorable_hand::{
	ScorableHand, ScorableHandChiitoi, ScorableHandFourthMeld, ScorableHandMeld, ScorableHandPair, ScorableHandRegular,
};
...
pub trait SortingNetwork: core::cmp::PartialOrd + Sized {
	fn sort_three(arr: &mut [Self; 3]);
}

impl SortingNetwork for u8 {
	fn sort_three(arr: &mut [Self; 3]) {
		...
		// while this three-swap version fits entirely in registers, has no branches, and is shorter to boot (three `maxu; minu` pairs on RV).
		//
		// Note that only some sequences of index pairs seems to optimize this way. Some sequences, like `[(0, 1), (1, 2), (0, 1)]` (bubble sort), don't optimize as well.
		//
		// Also, this copies the array locally, sorts it and then writes it back, instead of sorting the `&mut []` directly, because the latter generates loads and stores around every swap.
