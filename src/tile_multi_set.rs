...
// TODO(rustup): The `[(); TElement::N]: Sized` bounds in this file ought to be just `[(); TElement::N]:`, but that causes an ICE when compiling `--bin score`.
	...
	[(); TElement::N]: Sized,
{
	...
	///
	/// Returns `false` when inserting more of a tile than should exist.
	pub const fn insert_many(&mut self, tile: TElement::Tile, additional: usize) -> bool {
		let (mut count, max) = self.tile_to_count_max_mut(tile);
...
impl Tile34MultiSet {
	...
	pub const fn indicates_dora(&self, game_type: GameType) -> Self {
		let Self {
			counts_lo: [counts_lo_1m_8m, counts_lo_9m_7p, counts_lo_8p_6s, counts_lo_7s_5z, counts_lo_6z_7z],
			counts_hi: [counts_hi_1m_8m, counts_hi_9m_7p, counts_hi_8p_6s, counts_hi_7s_5z, counts_hi_6z_7z],
		} = self;

		let new_counts_lo_1m_8m =
			(counts_lo_1m_8m << 2) |
			(counts_lo_9m_7p & 0x0003);
		let new_counts_hi_1m_8m =
			(counts_hi_1m_8m << 1) |
			(counts_hi_9m_7p & 0b00000001);

		let new_counts_lo_9m_7p =
			((counts_lo_9m_7p & 0xFFFC) << 2) |
			(counts_lo_8p_6s & 0x000C) |
			match game_type { GameType::Yonma => counts_lo_1m_8m >> 14, GameType::Sanma => counts_lo_1m_8m & 0x0003 };
		let new_counts_hi_9m_7p =
			((counts_hi_9m_7p & 0b11111110) << 1) |
			(counts_hi_8p_6s & 0b00000010) |
			match game_type { GameType::Yonma => counts_hi_1m_8m >> 7, GameType::Sanma => counts_hi_1m_8m & 0b00000001 };

		let new_counts_lo_8p_6s =
			((counts_lo_8p_6s & 0xFFF3) << 2) |
			(counts_lo_7s_5z & 0x0030) |
			(counts_lo_9m_7p >> 14);
		let new_counts_hi_8p_6s =
			((counts_hi_8p_6s & 0b01111101) << 1) |
			(counts_hi_7s_5z & 0b00000100) |
			(counts_hi_9m_7p >> 7);

		let new_counts_lo_7s_5z =
			((counts_lo_7s_5z & 0x0FCF) << 2) |
			((counts_lo_6z_7z & 0x000C) << 12) |
			((counts_lo_7s_5z & 0x3000) >> 6) |
			(counts_lo_8p_6s >> 14);
		let new_counts_hi_7s_5z =
			((counts_hi_7s_5z & 0b00111011) << 1) |
			((counts_hi_6z_7z & 0b00000010) << 6) |
			((counts_hi_7s_5z & 0b01000000) >> 3) |
			(counts_hi_8p_6s >> 7);

		let new_counts_lo_6z_7z =
			((counts_lo_6z_7z & 0x0003) << 2) |
			(counts_lo_7s_5z >> 14);
		let new_counts_hi_6z_7z =
			((counts_hi_6z_7z & 0b00000001) << 1) |
			(counts_hi_7s_5z >> 7);

		Self {
			counts_lo: [new_counts_lo_1m_8m, new_counts_lo_9m_7p, new_counts_lo_8p_6s, new_counts_lo_7s_5z, new_counts_lo_6z_7z],
			counts_hi: [new_counts_hi_1m_8m, new_counts_hi_9m_7p, new_counts_hi_8p_6s, new_counts_hi_7s_5z, new_counts_hi_6z_7z],
		}
	}
}
...
mod tests {
	...
	fn indicates_dora() {
		for (input, expected_yonma, expected_sanma) in [
			...
			// 3 tests `counts_lo`, 4 tests `counts_hi`
			for n in [3, 4] {
				let input = [(input, n)].into_iter().collect::<Tile34MultiSet>();

				let actual = input.indicates_dora(GameType::Yonma);
				let expected_yonma = [(expected_yonma, n)].into_iter().collect::<Tile34MultiSet>();
				assert_eq!(actual, expected_yonma);

				let actual = input.indicates_dora(GameType::Sanma);
				let expected_sanma = expected_sanma.iter().map(|&t| (t, n)).collect::<Tile34MultiSet>();
