...
impl TileMultiSetElement for Tile27MultiSetElement {
	...
	fn tile_to_offset(tile: Self::Tile) -> (u8, usize) {
		// Similar technique as `Tile::cmp`, however here we do need to subtract `>=` instead of adding `<`, so there is an extra cost
		// to inverting the `sltiu` result on RV.
		let tile = tile as u8;
		let tile_diff = u8::from(tile >= tn!(0m) as u8) + u8::from(tile >= tn!(0p) as u8) + u8::from(tile >= tn!(0s) as u8);
		let offset = tile - tile_diff;
...
impl TileMultiSetElement for Tile34MultiSetElement {
	...
	fn tile_to_offset(tile: Self::Tile) -> (u8, usize) {
		...
		// to inverting the `sltiu` result on RV.
		let tile = tile as u8;
		let tile_diff = u8::from(tile >= t!(0m) as u8) + u8::from(tile >= t!(0p) as u8) + u8::from(tile >= t!(0s) as u8);
		let offset = tile - tile_diff;
	...
	TElement: TileMultiSetElement,
{
	...
	let count_lo = (counts_lo.as_slice()[offset_i] >> (offset_b * 2)) & 0b11;
	let count_hi = (counts_hi.as_slice()[offset_i] >> offset_b) & 0b1;
	(count_hi << 2 | (count_lo as u8)) as usize
}

fn tile_to_count_max_mut<'a, TElement>(
	tile: TElement::Tile,
	counts_lo: &'a mut GenericArray<u16, TElement::N>,
	counts_hi: &'a mut GenericArray<u8, TElement::N>,
) -> (U3x8Mut<'a>, usize)
where
	TElement: TileMultiSetElement,
{
...
mod tests {
	...
	fn all_27() {
		...
		assert!(set.into_iter().flat_map(|(t, n)| core::iter::repeat_n(t, n.get())).eq(NumberTile::all(GameType::Yonma).iter().copied()));
	}
