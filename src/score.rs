	...
	pub fn score(
		...
		match self {
			Self::Regular { melds: ([m1, m2, m3], m4), pair } => {
				...
				score.yakuhai_ton = Han(self.num_wind_yakuhai(tw!(E), round_wind, seat_wind));
				score.yakuhai_nan = Han(self.num_wind_yakuhai(tw!(S), round_wind, seat_wind));
				score.yakuhai_shaa = Han(self.num_wind_yakuhai(tw!(W), round_wind, seat_wind));
				score.yakuhai_pei = Han(self.num_wind_yakuhai(tw!(N), round_wind, seat_wind));
				score.yakuhai_haku = Han(if self.is_dragon_yakuhai(td!(Wh)) { 1 } else { 0 });
				score.yakuhai_hatsu = Han(if self.is_dragon_yakuhai(td!(G)) { 1 } else { 0 });
				score.yakuhai_chun = Han(if self.is_dragon_yakuhai(td!(R)) { 1 } else { 0 });
