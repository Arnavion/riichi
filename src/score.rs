...
pub enum WinningTileFrom {
	...
	Houtei(SeatRelative),
	/// The tile was drawn from the dead wall because the player called kan to create a daiminkan from the given player's discard.
	//
	// This is only used for rulesets that apply sekinin barai to rinshan. Other rulesets should use [`Rinshan`].
	RinshanPao(SeatRelative),
...
pub struct LocalYaku {
	...
	pub shiiaru_raotai: bool,
	pub kanburi: bool,
	pub toipuutao: bool,
	pub sanrenkou: bool,
	pub sanshoku_tsuukan: bool,
	pub akadora_sanshoku: bool,
	pub isshoku_sanjun: bool,
	pub uumensai: bool,
	pub iipin_mouyue: bool,
	pub chuupin_raoyui: bool,
	pub renhou: bool,
	pub ishi_no_ue_ni_mo_sannen: bool,
	pub suurenkou: bool,
	pub isshoku_yonjun: bool,
	pub hyakuman_goku: bool,
	pub kouitten: bool,
	pub benikujaku: bool,
	pub kokuiisou: bool,
	pub golden_gate_bridge: bool,
	pub touhoku_shinkansen: bool,
	pub dairin: bool,
	pub daichiishin: bool,
	/// When enabled, regular ryuuiisou is considered to require hatsu.
	pub chinryuusou: bool,
}
...
pub struct Score {
	...
	pub sanshoku_tsuukan: Han,

	pub akadora_sanshoku: Han,
	...
	pub suurenkou: Yakuman,

	pub daisharin: Yakuman,
	pub daichikurin: Yakuman,
	pub daisuurin: Yakuman,

	pub isshoku_yonjun: Yakuman,

	pub hyakuman_goku: Yakuman,

	pub kouitten: Yakuman,

	pub benikujaku: Yakuman,

	pub kokuiisou: Yakuman,

	pub golden_gate_bridge: Yakuman,

	pub touhoku_shinkansen: Yakuman,

	pub daichiishin: Yakuman,

	pub chinryuusou: Yakuman,
}
...
impl SeatRelative {
	/// Creates a `SeatRelative` for the `to` seat relative to the `from` seat.
...
impl WinningTileFrom {
	/// Returns the [`SeatRelative`] in this `WinningTileFrom`, if any.
	pub const fn ron_seat(self) -> Option<SeatRelative> {
		match self {
			Self::Haitei |
			Self::Rinshan |
			Self::Tenhou |
	...
	const fn tsubame_gaeshi(self) -> bool {
		match self {
			Self::Kanburi { tsubame_gaeshi, .. } |
			Self::Renhou { tsubame_gaeshi, .. } |
			Self::Ron { tsubame_gaeshi, .. } => tsubame_gaeshi,
			_ => false,
		}
	}
}
	...
	pub fn score(
		...
		match self {
			Self::Regular(h) => {
				...
				let peikou_isshoku_jun = h.peikou_isshoku_jun();
				let ittsuukansen = h.ittsuukansen();
				...
				score.tsumo = Fu(if matches!(tsumo_or_ron, TsumoOrRon::Tsumo) && !is_pinfu { 2 } else { 0 });

				score.menzen_kafu = Fu(if matches!(tsumo_or_ron, TsumoOrRon::Ron) && is_menzen { 10 } else { 0 });

				let mut fu = score.fu().0;

				score.open_pinfu = Fu(if !is_menzen && fu == 20 { fu += 2; 2 } else { 0 });
				...
				score.iipeikou = Han({
					let is_iipeikou =
						matches!(peikou_isshoku_jun, PeikouIsshokuJun::Iipeikou) ||
						(is_menzen && matches!(peikou_isshoku_jun, PeikouIsshokuJun::IsshokuSanjun | PeikouIsshokuJun::IsshokuYonjun));
					if is_iipeikou { 1 } else { 0 }
				});
				(score.haitei, score.iipin_mouyue) =
					if matches!(winning_tile_from, WinningTileFrom::Haitei) {
						if local_yaku.iipin_mouyue && winning_tile == t!(1p) {
							(Han(0), Han(5))
						}
						else {
							(Han(1), Han(0))
				...
				(score.houtei, score.chuupin_raoyui) =
					if matches!(winning_tile_from, WinningTileFrom::Houtei(_)) {
						if local_yaku.chuupin_raoyui && winning_tile == t!(9p) {
							(Han(0), Han(5))
						}
						else {
							(Han(1), Han(0))
				...
				score.rinshan = Han(if matches!(winning_tile_from, WinningTileFrom::Rinshan | WinningTileFrom::RinshanPao(_)) { 1 } else { 0 });
				...
				[score.yakuhai_ton, score.yakuhai_nan, score.yakuhai_shaa, score.yakuhai_pei] = h.num_wind_yakuhai(round_wind, seat_wind).map(Han);
				[score.yakuhai_haku, score.yakuhai_hatsu, score.yakuhai_chun] = h.is_dragon_yakuhai().map(|result| Han(if result { 1 } else { 0 }));

				score.tsubame_gaeshi = Han(if local_yaku.tsubame_gaeshi && winning_tile_from.tsubame_gaeshi() { 1 } else { 0 });
				score.shiiaru_raotai = Han(if local_yaku.shiiaru_raotai && h.is_shiiaru_raotai() { 1 } else { 0 });
				score.kanburi = Han(if local_yaku.kanburi && matches!(winning_tile_from, WinningTileFrom::Kanburi { .. }) { 1 } else { 0 });
				score.toipuutao = Han(if local_yaku.toipuutao && matches!(iisou, Iisou::Toipuutao) { 1 } else { 0 });
				...
				score.ittsuu = Han(if matches!(ittsuukansen, IttsuuKanSen::Ittsuu | IttsuuKanSen::TouhokuShinkansen) { if is_menzen { 2 } else { 1 } } else { 0 });
				score.toitoi = Han(if h.is_toitoi() { 2 } else { 0 });
				score.sanankou = Han(if matches!(num_ankou, NumAnkou::Sanankou) { 2 } else { 0 });
				score.sanshoku_doukou = Han(if h.is_sanshoku_doukou() { 2 } else { 0 });
				score.sankantsu = Han(if matches!(num_kantsu, NumKantsu::Sankantsu) { 2 } else { 0 });
				score.honroutou = Han(if chanta_routou.is_honroutou() { 2 } else { 0 });
				score.shousangen = Han(if matches!(suushii_sangen, SuushiiSangen::Shousangen) { 2 } else { 0 });

				score.sanrenkou = Han(if local_yaku.sanrenkou && matches!(num_renkou, NumRenkou::Sanrenkou | NumRenkou::Suurenkou) { 2 } else { 0 });
				score.sanshoku_tsuukan = Han(if local_yaku.sanshoku_tsuukan && matches!(ittsuukansen, IttsuuKanSen::SanshokuTsuukan) { if is_menzen { 2 } else { 1 } } else { 0 });
				...
				if local_yaku.isshoku_sanjun && matches!(peikou_isshoku_jun, PeikouIsshokuJun::IsshokuSanjun | PeikouIsshokuJun::IsshokuYonjun) {
					score.isshoku_sanjun = Han(if is_menzen { 3 } else { 2 });
				...
				score.shousuushii = Yakuman(if matches!(suushii_sangen, SuushiiSangen::Shousuushii) { 1 } else { 0 });
				score.daisuushii = Yakuman(if matches!(suushii_sangen, SuushiiSangen::Daisuushii) { 2 } else { 0 });
				...
				score.ryuuiisou = Yakuman(if matches!(iisou, Iisou::Ryuuiisou | Iisou::Chinryuusou) { 1 } else { 0 });
				score.chuuren_poutou = Yakuman(h.num_chuuren_poutou());
				score.suukantsu = Yakuman(if matches!(num_kantsu, NumKantsu::Suukantsu) { 1 } else { 0 });

				if local_yaku.suurenkou && matches!(num_renkou, NumRenkou::Suurenkou) {
					score.suurenkou = Yakuman(1);
					score.sanrenkou = Han(0);
				}
				if local_yaku.isshoku_yonjun && matches!(peikou_isshoku_jun, PeikouIsshokuJun::IsshokuYonjun) {
					score.isshoku_yonjun = Yakuman(1);
					score.isshoku_sanjun = Han(0);
					score.iipeikou = Han(0);
				}
				score.hyakuman_goku = Yakuman(if local_yaku.hyakuman_goku && h.is_hyakuman_goku() { 1 } else { 0 });
				score.kouitten = Yakuman(if local_yaku.kouitten && matches!(iisou, Iisou::Kouitten) { 1 } else { 0 });
				score.benikujaku = Yakuman(if local_yaku.benikujaku && matches!(iisou, Iisou::Benikujaku) { 1 } else { 0 });
				score.kokuiisou = Yakuman(if local_yaku.kokuiisou && matches!(iisou, Iisou::Kokuiisou) { 1 } else { 0 });
				score.golden_gate_bridge = Yakuman(if local_yaku.golden_gate_bridge && h.is_golden_gate_bridge() { 1 } else { 0 });
				if local_yaku.touhoku_shinkansen && matches!(ittsuukansen, IttsuuKanSen::TouhokuShinkansen) {
					score.touhoku_shinkansen = Yakuman(1);
					score.ittsuu = Han(0);
				}

				if local_yaku.chinryuusou && matches!(iisou, Iisou::Chinryuusou) {
					score.chinryuusou = Yakuman(2);
					score.ryuuiisou = Yakuman(0);
				}
			},
			...
			Self::Chiitoi(h) => {
				let chanta_routou = h.chanta_routou();
				let honchinitsu = h.honchinitsu();
				let dairin_kokuiisou = h.dairin_kokuiisou();
				...
				score.tsubame_gaeshi = Han(if local_yaku.tsubame_gaeshi && winning_tile_from.tsubame_gaeshi() { 1 } else { 0 });
				score.kanburi = Han(if local_yaku.kanburi && matches!(winning_tile_from, WinningTileFrom::Kanburi { .. }) { 1 } else { 0 });
				score.toipuutao = Han(if local_yaku.toipuutao && h.is_toipuutao() { 1 } else { 0 });
				...
				score.daisharin = Yakuman(if local_yaku.dairin && matches!(dairin_kokuiisou, DairinKokuiisou::Daisharin) { 1 } else { 0 });
				score.daichikurin = Yakuman(if local_yaku.dairin && matches!(dairin_kokuiisou, DairinKokuiisou::Daichikurin) { 1 } else { 0 });
				score.daisuurin = Yakuman(if local_yaku.dairin && matches!(dairin_kokuiisou, DairinKokuiisou::Daisuurin) { 1 } else { 0 });

				score.kokuiisou = Yakuman(if local_yaku.kokuiisou && matches!(dairin_kokuiisou, DairinKokuiisou::Kokuiisou) { 1 } else { 0 });

				if local_yaku.dairin && matches!(dairin_kokuiisou, DairinKokuiisou::Daichiishin) {
					score.daichiishin = Yakuman(2);
					score.tsuuiisou = Yakuman(0);
				}
			},
		...
		score.renhou = Yakuman(if local_yaku.renhou && matches!(winning_tile_from, WinningTileFrom::Renhou { .. }) { 1 } else { 0 });

		score.ishi_no_ue_ni_mo_sannen = Yakuman(
			if local_yaku.ishi_no_ue_ni_mo_sannen && matches!(riichi, Riichi::Riichi { double: true, .. }) && matches!(winning_tile_from, WinningTileFrom::Haitei | WinningTileFrom::Houtei(_)) { 1 }
			else { 0 }
		);
...
impl ScorableHandRegular {
	fn melds_fu(&self) -> [Fu; 4] {
		...
		let masks = Tile::masks(Tile::offsets(ts));
...
impl ScorableHandFourthMeld {
	const fn wait_fu(self) -> Fu {
		...
		let wait_fu = (0b00_00_00_00_11_00_11_00_111111_0_u32 >> (self.dw() as u8)) & 0b10;
		Fu(wait_fu as u8)
	}
}
...
impl Score {
	const NUM_FU_FIELDS: usize = 11;
	const NUM_HAN_FIELDS: usize = 44;
	const NUM_YAKUMAN_FIELDS: usize = 28;
	...
	pub fn score(
		...
		honba: u8,
		seat_wind: WindTile,
		ron_seat: Option<SeatRelative>,
		daisangen_sekinin_barai: Option<SeatRelative>,
		daisuushii_sekinin_barai: Option<SeatRelative>,
	) -> Points {
		fn score_inner(points: &mut Points, seat_wind: WindTile, ron_seat: Option<SeatRelative>, basic_points: u32) {
			if let Some(seat) = ron_seat {
				// If dealer, 6x from ron'd player.
				// If non-dealer, 4x from ron'd player.
				let ron = (basic_points * if seat_wind == tw!(E) { 6 } else { 4 }).next_multiple_of(100);
				points[seat] += ron;
			}
			else {
		...
		fn sekinin_barai_inner(points: &mut Points, seat_wind: WindTile, ron_seat: Option<SeatRelative>, sekinin_barai: SeatRelative, yakuman: u8) {
			let basic_points = u32::from(yakuman) * 8000;
			let ron_half = basic_points * if seat_wind == tw!(E) { 3 } else { 2 };
			if let Some(seat) = ron_seat {
				...
				points[seat] += ron_half;
				points[sekinin_barai] += ron_half;
			}
			else {
				...
				points[sekinin_barai] += ron_half * 2;
			}
		}
		...
		if self.yakuman().0 == 0 {
			...
			score_inner(&mut points, seat_wind, ron_seat, basic_points);
			...
			for yakuman in [
				kazoe_yakuman, kokushi_musou, suuankou, shousuushii, tsuuiisou, chinroutou, ryuuiisou, chuuren_poutou, suukantsu, tenhou, chiihou,
				renhou, ishi_no_ue_ni_mo_sannen, suurenkou, daisharin, daichikurin, daisuurin, isshoku_yonjun, hyakuman_goku, kouitten, benikujaku, kokuiisou, golden_gate_bridge, touhoku_shinkansen,
				daichiishin, chinryuusou,
			] {
				score_inner(&mut points, seat_wind, ron_seat, u32::from(yakuman.0) * 8000);
			}
			if let Some(sekinin_barai) = daisangen_sekinin_barai {
				sekinin_barai_inner(&mut points, seat_wind, ron_seat, sekinin_barai, daisangen.0);
			}
			else {
				score_inner(&mut points, seat_wind, ron_seat, u32::from(daisangen.0) * 8000);
			}
			if let Some(sekinin_barai) = daisuushii_sekinin_barai {
				sekinin_barai_inner(&mut points, seat_wind, ron_seat, sekinin_barai, daisuushii.0);
			}
			else {
				score_inner(&mut points, seat_wind, ron_seat, u32::from(daisuushii.0) * 8000);
			}

			// Sekinin barai player pays full honba penalty regardless of tsumo or ron.
			let honba_seat = daisangen_sekinin_barai.or(daisuushii_sekinin_barai).or(ron_seat);
		...
		if matches!(game_type, GameType::Sanma) && let Some(north_seat) = SeatRelative::of(seat_wind, tw!(N)) {
			points[north_seat] = 0;
		}
...
impl core::fmt::Debug for Score {
	fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
		...
		fmt_inner!(
			...
			wait, tsumo, menzen_kafu, open_pinfu,
			rounding,
			...
			tsubame_gaeshi, shiiaru_raotai, kanburi, toipuutao,
			chanta, sanshoku_doujun, ittsuu,
			toitoi, sanankou, sanshoku_doukou, sankantsu, chiitoi, honroutou, shousangen,
			sanrenkou, sanshoku_tsuukan, akadora_sanshoku, uumensai,
...
#[repr(C)]
struct ScoreParts {
	fu: [u8; Score::NUM_FU_FIELDS],
	han: [u8; Score::NUM_HAN_FIELDS],
	yakuman: [u8; Score::NUM_YAKUMAN_FIELDS],
}

assert_size_of!(Score, core::mem::size_of::<ScoreParts>());
...
impl LocalYaku {
	...
	pub const fn riichi_city() -> Self {
		Self {
			...
			toipuutao: true,
			sanrenkou: true,
			sanshoku_tsuukan: true,
			akadora_sanshoku: true,
			uumensai: true,
			isshoku_sanjun: true,
			renhou: true,
			ishi_no_ue_ni_mo_sannen: true,
			...
			benikujaku: true,
			kokuiisou: true,
			golden_gate_bridge: true,
			touhoku_shinkansen: true,
			daichiishin: true,
			chinryuusou: true,
			..Default::default()
		}
...
const impl core::ops::Index<SeatRelative> for Points {
	...
	fn index(&self, seat: SeatRelative) -> &Self::Output {
		unsafe { &*(&raw const self.shimocha).add(seat as usize) }
	}
}
...
/// Convenience function to take the given sequence of [`ScorableHand`]s and return the one with the highest aggregate score.
///
/// Returns `None` if none of the given hands have any yaku.
///
/// This is a wrapper around [`Hand::to_scorable_hands`](crate::Hand::to_scorable_hands) and [`ScorableHand::score`].
...
) -> Option<(ScorableHand, Score, ScoreAggregate)> {
	hands.into_iter()
		.filter_map(|hand| {
			...
			let score_aggregate = ScoreAggregate::from(score);
			Some((hand, score, score_aggregate))
		})
		.max_by(|&(_, score1, score_aggregate1), &(_, score2, score_aggregate2)|
			score_aggregate1.yakuman.0.cmp(&score_aggregate2.yakuman.0)
			.then_with(|| score_aggregate1.han.0.cmp(&score_aggregate2.han.0))
			.then_with(|| score_aggregate1.fu.0.cmp(&score_aggregate2.fu.0))
			.then_with(|| score1.rounding.0.cmp(&score2.rounding.0).reverse())
		)
}
...
mod tests {
	...
	#[test]
	fn seat_relative_of() {
		for (seat_wind, target, expected) in [
			(tw!(E), tw!(E), None),
			(tw!(E), tw!(S), Some(SeatRelative::Shimocha)),
			(tw!(E), tw!(W), Some(SeatRelative::Toimen)),
			(tw!(E), tw!(N), Some(SeatRelative::Kamicha)),
			(tw!(S), tw!(E), Some(SeatRelative::Kamicha)),
			(tw!(S), tw!(S), None),
			(tw!(S), tw!(W), Some(SeatRelative::Shimocha)),
			(tw!(S), tw!(N), Some(SeatRelative::Toimen)),
			(tw!(W), tw!(E), Some(SeatRelative::Toimen)),
			(tw!(W), tw!(S), Some(SeatRelative::Kamicha)),
			(tw!(W), tw!(W), None),
			(tw!(W), tw!(N), Some(SeatRelative::Shimocha)),
			(tw!(N), tw!(E), Some(SeatRelative::Shimocha)),
			(tw!(N), tw!(S), Some(SeatRelative::Toimen)),
			(tw!(N), tw!(W), Some(SeatRelative::Kamicha)),
			(tw!(N), tw!(N), None),
		] {
			let actual = SeatRelative::of(seat_wind, target);
			assert_eq!(actual, expected);
		}
	}

	// Ref: https://riichi.wiki/index.php?title=Japanese_mahjong_scoring_rules&oldid=30869
	//
	...
	fn score_rounding() {
		...
		let points = score.score(GameType::Yonma, 0, 0, tw!(S), Some(SeatRelative::Shimocha), None, None);
		...
		let points = score.score(GameType::Yonma, 0, 0, tw!(S), None, None, None);
	...
	fn scoring_rules1() {
		...
		let winning_tile_from = WinningTileFrom::Ron { seat: SeatRelative::Shimocha, tsubame_gaeshi: false };
		...
		assert_eq!(
			score.score(game_type, 0, 0, seat_wind, winning_tile_from.ron_seat(), None, None).total(),
			3900,
		);
	}
	...
	// Ref: Riichi City: Mahjong Academy -> Lexicon -> High Score Rule
	//
	// Chiitoi vs ryanpeikou
	#[test]
	fn high_score_rule1() {
		let winning_tile = t!(8s);
		let winning_tile_from = WinningTileFrom::Ron { seat: SeatRelative::Toimen, tsubame_gaeshi: false };
		let hand = make_hand!(G G 7m 7m 8m 8m 9m 9m 6s 6s 7s 7s 8s);
		let hands = hand.to_scorable_hands(winning_tile, winning_tile_from.into());
		let (hand, score, _) = max_score(
			&Default::default(),
			&Default::default(),
			tw!(E),
			tw!(E),
		...
		assert_eq!(hand, make_scorable_hand!({ anjun 7m 8m 9m } { anjun 7m 8m 9m } { anjun 6s 7s 8s } { minjun 6s 7s 8s ryanmen_high } { G G }));
		assert_eq!(score, Score {
			base: Fu(20),
			pair: Fu(2),
			menzen_kafu: Fu(10),
	...
	// Ref: Riichi City: Mahjong Academy -> Lexicon -> High Score Rule
	//
	// Pinfu vs sanshoku doujun
	#[test]
	fn high_score_rule2() {
		let winning_tile = t!(6s);
		let winning_tile_from = WinningTileFrom::Ron { seat: SeatRelative::Toimen, tsubame_gaeshi: false };
		let hand = make_hand!(5m 6m 7m 5p 6p 7p 4s 4s 5s 5s 6s 7s 7s);
		let hands = hand.to_scorable_hands(winning_tile, winning_tile_from.into());
		let (hand, score, _) = max_score(
			&Default::default(),
			&Default::default(),
		...
		assert_eq!(hand, make_scorable_hand!({ anjun 5m 6m 7m } { anjun 5p 6p 7p } { anjun 5s 6s 7s } { minjun 5s 6s 7s kanchan } { 4s 4s }));
		assert_eq!(score, Score {
			base: Fu(20),
			wait: Fu(2),
			menzen_kafu: Fu(10),
	...
	#[test]
	fn high_score_rule3() {
		let winning_tile = t!(7p);
		let winning_tile_from = WinningTileFrom::Ron { seat: SeatRelative::Toimen, tsubame_gaeshi: false };
		let hand = make_hand!(1p 2p 3p 4p 4p 5p 5p 6p 6p 7p 7p 8p 9p);
		let hands = hand.to_scorable_hands(winning_tile, winning_tile_from.into());
		let (hand, score, _) = max_score(
			&Default::default(),
			&Default::default(),
		...
		assert_eq!(hand, make_scorable_hand!({ anjun 1p 2p 3p } { anjun 4p 5p 6p } { anjun 4p 5p 6p } { minjun 7p 8p 9p penchan } { 7p 7p }));
		assert_eq!(score, Score {
			base: Fu(20),
			wait: Fu(2),
			menzen_kafu: Fu(10),
	...
	// Ref: Riichi City: Mahjong Academy -> Lexicon -> High Score Rule
	//
	// Sanankou vs junchan + pinfu + iipeikou
	#[test]
	fn high_score_rule4() {
		let winning_tile = t!(9m);
		let winning_tile_from = WinningTileFrom::Ron { seat: SeatRelative::Toimen, tsubame_gaeshi: false };
		let hand = make_hand!(1p 1p 1p 2p 2p 2p 3p 3p 3p 7m 8m 9m 9m);
		let hands = hand.to_scorable_hands(winning_tile, winning_tile_from.into());
		let (hand, score, _) = max_score(
			&Default::default(),
			&Default::default(),
		...
		assert_eq!(hand, make_scorable_hand!({ anjun 1p 2p 3p } { anjun 1p 2p 3p } { anjun 1p 2p 3p } { minjun 7m 8m 9m ryanmen_high } { 9m 9m }));
		assert_eq!(score, Score {
			base: Fu(20),
			menzen_kafu: Fu(10),
			pinfu: Han(1),
	...
	// Ref: Riichi City: Mahjong Academy -> Lexicon -> High Score Rule
	//
	// Ryanmen vs kanchan, ron
	#[test]
	fn high_score_rule5() {
		let winning_tile = t!(2m);
		let winning_tile_from = WinningTileFrom::Ron { seat: SeatRelative::Toimen, tsubame_gaeshi: false };
		let hand = make_hand!(1m 2m 3m 3m 4m 6m 6m 7p 7p 7p { minkou N N N });
		let hands = hand.to_scorable_hands(winning_tile, winning_tile_from.into());
		let (hand, score, _) = max_score(
			&Default::default(),
			&Default::default(),
		...
		assert_eq!(hand, make_scorable_hand!({ anjun 2m 3m 4m } { ankou 7p 7p 7p } { minkou N N N } { minjun 1m 2m 3m kanchan } { 6m 6m }));
		assert_eq!(score, Score {
			base: Fu(20),
			meld2: Fu(4),
			meld3: Fu(4),
	...
	fn high_score_rule6() {
		...
		assert_eq!(hand, make_scorable_hand!({ anjun 2m 3m 4m } { ankou 7p 7p 7p } { minkou N N N } { anjun 1m 2m 3m kanchan } { 6m 6m }));
		assert_eq!(score, Score {
			base: Fu(20),
			meld2: Fu(4),
			meld3: Fu(4),
		...
		assert_eq!(ScoreAggregate::from(score), ScoreAggregate { fu: Fu(40), han: Han(1), yakuman: Yakuman(0) });
	}
	...
	fn honroutou1() {
		...
		let hands: std::vec::Vec<_> = t![1p, 9s].into_iter().flat_map(|winning_tile| {
			let hands: std::collections::BTreeSet<_> = hand.clone().to_scorable_hands(winning_tile, winning_tile_from.into()).collect();
			assert_eq!(hands.len(), 1);
	...
	fn kazoe_yakuman2() {
...

		{
			...
			let mut hands: std::collections::BTreeSet<_> = hand.clone().to_scorable_hands(winning_tile, winning_tile_from.into()).collect();
			assert_eq!(hands.len(), 1);
			let hand = hands.pop_first().unwrap();
	...
	//      from https://riichi.wiki/index.php?title=Mahjong_Soul&oldid=30901#Rules
	#[test]
	fn composite_yakuman() {
		...
		assert_eq!(
			score.score(game_type, 0, 0, seat_wind, winning_tile_from.ron_seat(), None, Some(SeatRelative::Shimocha)),
			Points { shimocha: 112000, toimen: 16000, kamicha: 16000, ..Default::default() },
		);
	}

	// Ref: https://mahjongsoul.game.yo-star.com/?paipu=200108-a85dc57e-3512-4995-81c0-18874087c8dc_a61983483
	//      from https://riichi.wiki/index.php?title=Mahjong_Soul&oldid=30901#Rules
	#[test]
	fn tenhou() {
		let game_type = GameType::Sanma;
		let doras = t34multiset![4s].indicates_dora(game_type);
		let round_wind = tw!(E);
		let seat_wind = tw!(E);
		let hand = make_hand!(1m 9m 1p 9p 9p 1s 9s E S W N Wh G R);
		let winning_tile = t!(R);
		let winning_tile_from = WinningTileFrom::Tenhou;
		let mut hands: std::collections::BTreeSet<_> = hand.to_scorable_hands().collect();
		assert_eq!(hands.len(), 1);
		let hand = hands.pop_first().unwrap();
		assert_eq!(hand, make_scorable_hand!(1m 9m 1p 9p 9p 1s 9s E S W N Wh G R juusanmen));
		let score = hand.score(
			&Default::default(),
			&doras,
			round_wind,
		...
		assert_eq!(score, Score {
			tenhou: Yakuman(1),
			kokushi_musou: Yakuman(2),
			..Default::default()
		});
		assert_eq!(
			score.score(game_type, 0, 1, seat_wind, winning_tile_from.ron_seat(), None, None),
			Points { shimocha: 48100, toimen: 48100, ..Default::default() },
		);
	}
