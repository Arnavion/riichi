	...
	pub fn score(
		...
		match self {
			Self::Regular(h) => {
				...
				score.shousangen = Han(if h.is_shousangen() { 2 } else { 0 });
				...
				score.daisangen = Yakuman(if h.is_daisangen() { 1 } else { 0 });
				score.shousuushii = Yakuman(if h.is_shousuushii() { 1 } else { 0 });
				score.daisuushii = Yakuman(if h.is_daisuushii() { 2 } else { 0 });
...
impl ScorableHandMeld {
	const fn fu(self) -> Fu {
		...
		let (base, t) = match self {
			...
			Self::Minkou(t) => (2, t),
			Self::Anjun { tile, .. } |
			Self::Minjun { tile, .. } => (0, Tile::const_from_slt(tile)),
		};
