use crate::{
	...
	WindTile,
	scorable_hand::{Dairin, Iisou, IttsuuKanSen, NumAnkou, NumKantsu, NumRenkou, PeikouIsshokuJun, SuushiiSangen},
};
...
pub enum WinningTileFrom {
	...
	/// The tile was taken from another player's discard when that player called kan and then riichi.
	KanburiTsubameGaeshi(SeatRelative),
...
impl WinningTileFrom {
	const fn ron_seat(self) -> Option<SeatRelative> {
		match self {
			...
			Self::Kanburi(seat) |
			Self::KanburiTsubameGaeshi(seat) |
			Self::Renhou(seat) |
	...
	pub fn score(
		...
		match self {
			Self::Regular(h) => {
				...
				score.tsubame_gaeshi = Han(if local_yaku.tsubame_gaeshi && matches!(winning_tile_from, WinningTileFrom::TsubameGaeshi(_) | WinningTileFrom::KanburiTsubameGaeshi(_)) { 1 } else { 0 });
				score.shiiaru_raotai = Han(if local_yaku.shiiaru_raotai && h.is_shiiaru_raotai() { 1 } else { 0 });
				score.kanburi = Han(if local_yaku.kanburi && matches!(winning_tile_from, WinningTileFrom::Kanburi(_) | WinningTileFrom::KanburiTsubameGaeshi(_)) { 1 } else { 0 });
			...
			Self::Chiitoi(h) => {
				...
				let chanta_routou = h.chanta_routou();
				let honchinitsu = h.honchinitsu();
				let dairin = if local_yaku.dairin { h.dairin() } else { Dairin::None };
				...
				score.kokuiisou = Yakuman(if local_yaku.kokuiisou && h.is_kokuiisou() { 1 } else { 0 });

				if matches!(dairin, Dairin::Ji) {
					score.daichisei = Yakuman(2);
		...
		score.renhou = Yakuman(if local_yaku.renhou && matches!(winning_tile_from, WinningTileFrom::Renhou(_)) { 1 } else { 0 });
...
mod tests {
	...
	// Ref: https://riichi.wiki/index.php?title=Japanese_mahjong_scoring_rules&oldid=30625
	//
	...
	fn high_score_rule1() {
		...
		let winning_tile_from = WinningTileFrom::Ron(SeatRelative::Toimen);
	...
	//      from https://riichi.wiki/index.php?title=Mahjong_Soul&oldid=30696#Rules
	#[test]
	...
	fn tenhou() {
		...
		assert_eq!(score.score(game_type, 0, 1, seat_wind, winning_tile_from, None, None), Points { shimocha: 48100, toimen: 48100, ..Default::default() });
	}
