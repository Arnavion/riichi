...
pub struct ScoreRaw {
	...
	/// 10 for menzenchin ron, 2 for tsumo except if pinfu.
	...
	/// 2 fu for a non-menzenchin hand with no fu other than 20 base fu.
	pub open_pinfu: Fu,

	pub menzenchin_tsumohou: Han,
...
impl ScorableHand {
	...
	pub fn score(
		...
		let is_menzenchin = self.is_menzenchin();
		...
		let m4 = match self {
			Self::Regular { melds: ([m1, m2, m3], m4), pair } => {
				...
				match tsumo_or_ron {
					...
					TsumoOrRon::Ron if is_menzenchin => score.win_condition = Fu(10),
					_ => (),
				}
				...
				score.open_pinfu = Fu(if !is_menzenchin && Score::from(score).fu == 20 { 2 } else { 0 });

				score.menzenchin_tsumohou = Han(if is_menzenchin && matches!(tsumo_or_ron, TsumoOrRon::Tsumo) { 1 } else { 0 });
				...
				score.rinshan_kaihou = Han(if matches!(winning_tile_from, WinningTileFrom::RinshanKaihou) { 1 } else { 0 });
				...
				score.chanta = Han(if self.is_chanta() { if is_menzenchin { 2 } else { 1 } } else { 0 });
				score.sanshoku_doujun = Han(if self.is_sanshoku_doujun() { if is_menzenchin { 2 } else { 1 } } else { 0 });
				score.ittsuu = Han(if self.is_ittsuu() { if is_menzenchin { 2 } else { 1 } } else { 0 });
				...
				score.honitsu = Han(if self.is_honitsu() { if is_menzenchin { 3 } else { 2 } } else { 0 });
				score.junchan = Han(if self.is_junchan() { if is_menzenchin { 3 } else { 2 } } else { 0 });
				score.ryanpeikou = Han(if self.is_ryanpeikou() { 3 } else { 0 });

				score.chinitsu = Han(if self.is_chinitsu() { if is_menzenchin { 6 } else { 5 } } else { 0 });
		...
		let dora = {
			...
			(0..nukidora).map(|_| dora_match(&doras, Tile::Wind(WindTile::Pei))).sum::<u32>()
		};
		score.dora = Han(dora);
		score.nukidora = Han(nukidora);

		if let Some(m4) = m4 {
			score.suuankou = Yakuman(if self.is_suuankou() { if matches!(m4, ScorableHandFourthMeld::Tanki(_)) { 2 } else { 1 } } else { 0 });
		}
		...
		score.chuuren_poutou = Yakuman(self.is_chuuren_poutou());
...
impl ScorableHandMeld {
	const fn fu(&self) -> u32 {
		match self {
			Self::Anjun(_) |
			Self::Minjun(_) => 0,
			Self::Ankou([t, ..]) => if t.is_simple() { 4 } else { 8 },
			Self::Minkou([t, ..]) => if t.is_simple() { 2 } else { 4 },
			Self::Ankan([t, ..]) => if t.is_simple() { 16 } else { 32 },
			Self::Minkan([t, ..]) => if t.is_simple() { 8 } else { 16 },
		}
	}
}
...
fn dora_match(doras: &[Tile], tile: Tile) -> u32 {
	let result =
		usize::from(matches!(tile, Tile::Number(NumberTile { number: Number::FiveRed, .. }))) +
...
mod tests {
	...
	fn score_rounding() {
		...
		assert_eq!(Score { fu: 30, han: 1, yakuman: 0 }.score(false, TsumoOrRon::Ron).total(false, GameType::FourPlayer), 1000);
		assert_eq!(Score { fu: 30, han: 1, yakuman: 0 }.score(false, TsumoOrRon::Tsumo).total(false, GameType::FourPlayer), 1100);
	}
	...
	fn honroutou() {
		...
		let winning_tile_from = WinningTileFrom::Ron;
		let hands: Vec<_> = [t!(1p), t!(9s)].into_iter().map(|new_tile| {
			let mut hands = hand.to_scorable_hands(new_tile, winning_tile_from.into());
			assert_eq!(hands.len(), 1);
			hands.pop_first().unwrap()
		}).collect();
		...
		for hand in hands {
			...
			let score = hand.score(
				...
				[],
				tw!(E),
				tw!(E), // Unknown what the seat wind is, but it's definitely not S
			).unwrap();
			assert_eq!(score, ScoreRaw {
				base: Fu(20),
