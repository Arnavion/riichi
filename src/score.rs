			...
			Self::Regular(h) => {
				...
				(score.haitei, score.iipin_mouyue) =
					if matches!(winning_tile_from, WinningTileFrom::Haitei) {
						if local_yaku && h.winning_tile() == t!(1p) {
							(Han(0), Han(5))
						}
						else {
							(Han(1), Han(0))
				...
				(score.haitei, score.chuupin_raoyui) =
					if matches!(winning_tile_from, WinningTileFrom::Haitei) {
						if local_yaku && h.winning_tile() == t!(9p) {
							(Han(0), Han(5))
						}
						else {
							(Han(1), Han(0))
				...
				score.isshoku_sanjun = Han(if local_yaku && is_isshoku_sanjun { 2 } else { 0 });
...
impl Score {
	...
	fn han(&self) -> Han {
		self.mentsumo + self.riichi + self.ippatsu + self.pinfu + self.iipeikou +
			...
			self.tsubame_gaeshi + self.kanburi + self.shiiaru_raotai +
			...
			self.uumensai + self.sanrenkou +
			self.honitsu + self.junchan + self.ryanpeikou +
			self.isshoku_sanjun + self.iipin_mouyue + self.chuupin_raoyui +
			self.chinitsu +
	...
	fn yakuman(&self) -> Yakuman {
		...
		self.renhou + self.daisharin + self.daichikurin + self.daisuurin + self.ishi_no_ue_ni_mo_sannen + self.daichisei
	}
}
...
impl core::fmt::Display for Yakuman {
	fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
		...
		// Making a single string and indexing it manually replaces the jump table with a LUT.
		const STRINGS: &str = "\
			\
			Yakuman\
			Double yakuman\
		...
		let (start, len) = START_AND_LENS[usize::from(self.0).min(START_AND_LENS.len() - 1)];
