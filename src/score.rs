...
pub enum WinningTileFrom {
	...
	/// The tile was taken from another player's shouminkan or daiminkan.
	ChankanMinkan(SeatRelative),
	...
	pub fn score(
		...
		match self {
			Self::Regular { melds: ([m1, m2, m3], m4), pair } => {
				...
				let is_pinfu = self.is_pinfu(round_wind, seat_wind);
				...
				score.meld2 = m2.fu();
				score.meld3 = m3.fu();
				(score.meld4, score.wait) = m4.fu();
				score.pair = pair.fu(round_wind, seat_wind);
				...
				score.chankan = Han(match winning_tile_from {
					...
					WinningTileFrom::ChankanMinkan(_) => 1,
					_ => 0,
				});
				score.tanyao = Han(if self.is_tanyao() { 1 } else { 0 });
				score.yakuhai_ton = Han(self.num_wind_yakuhai(WindTile::Ton, round_wind, seat_wind));
				score.yakuhai_nan = Han(self.num_wind_yakuhai(WindTile::Nan, round_wind, seat_wind));
				score.yakuhai_shaa = Han(self.num_wind_yakuhai(WindTile::Shaa, round_wind, seat_wind));
				score.yakuhai_pei = Han(self.num_wind_yakuhai(WindTile::Pei, round_wind, seat_wind));
		...
		let dora = {
			self.into_iter().map(|tile| dora_match(doras, tile)).sum::<u8>() +
			(0..nukidora).map(|_| dora_match(doras, t!(N))).sum::<u8>()
		};
		score.dora = Han(dora);
...
const impl From<WinningTileFrom> for TsumoOrRon {
	fn from(wtf: WinningTileFrom) -> Self {
		match wtf {
			...
			WinningTileFrom::ChankanMinkan(_) |
...
mod tests {
	...
	fn kazoe_yakuman() {
		...
		let winning_tile_from = WinningTileFrom::Tsumo;
		let mut hands = hand.to_scorable_hands(t!(1p), winning_tile_from.into());
		assert_eq!(hands.len(), 2);
		let hand = hands.pop_first().unwrap();
		assert_eq!(hand, make_scorable_hand!({ ankou 1p 1p 1p } { ankou 5p 5p 0p } { ankou 7p 7p 7p } { anjun 1p 2p 3p ryanmen_left } { 9p 9p }));
		let hand = hands.pop_first().unwrap();
		...
		let winning_tile_from = WinningTileFrom::Ron(SeatRelative::Shimocha);
		let mut hands = hand.to_scorable_hands(t!(1m), winning_tile_from.into());
		assert_eq!(hands.len(), 1);
		let hand = hands.pop_first().unwrap();
		...
		let hand = make_hand!(3p 3p 3p 6p 7p 8p 1m 1m 6m 6m { ankan 5m 0m 5m 5m });
		let winning_tile_from = WinningTileFrom::Ron(SeatRelative::Shimocha);
		let mut hands = hand.to_scorable_hands(t!(6m), winning_tile_from.into());
		assert_eq!(hands.len(), 1);
		let hand = hands.pop_first().unwrap();
		...
		let winning_tile_from = WinningTileFrom::Tsumo;
		let mut hands = hand.to_scorable_hands(t!(7s), winning_tile_from.into());
		assert_eq!(hands.len(), 4);
		let hand = hands.pop_first().unwrap();
		assert_eq!(hand, make_scorable_hand!({ anjun 2s 3s 4s } { anjun 2s 3s 4s } { anjun 5s 6s 7s } { anjun 5s 6s 7s ryanmen_right } { 8s 8s }));
		let score = hand.score(
			&[],
			tw!(E),
			tw!(E),
		...
		let winning_tile_from = WinningTileFrom::Ron(SeatRelative::Shimocha);
		let mut hands = hand.to_scorable_hands(t!(R), winning_tile_from.into());
		assert_eq!(hands.len(), 1);
		let hand = hands.pop_first().unwrap();
		...
		let hand = hand.to_scorable_hands(t!(G)).unwrap();
	...
	fn composite_yakuman() {
		...
		let winning_tile_from = WinningTileFrom::Tsumo;
		let hand = hand.to_scorable_hands(t!(R)).unwrap();
