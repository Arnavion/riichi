...
impl ScorableHandRegular {
	pub fn new(ma: ScorableHandMeld, mb: ScorableHandMeld, mc: ScorableHandMeld, md: ScorableHandFourthMeld, pair: ScorableHandPair) -> Self {
		...
		Self { m1, m2, m3, m4, pair }
	}

	fn for_each_tile(&self, mut f: impl FnMut(Tile)) {
		let Self { m1, m2, m3, m4, pair } = self;
		m1.for_each_tile(&mut f);
		m2.for_each_tile(&mut f);
		m3.for_each_tile(&mut f);
		m4.for_each_tile(&mut f);
		pair.for_each_tile(f);
	}
	...
	pub(crate) fn num_peikou(&self) -> usize {
		...
		let t = match m4 {
			...
			ScorableHandFourthMeld::Minkou(_, KouWait::Shanpon) => None,

			ScorableHandFourthMeld::Anjun(t, _) |
			ScorableHandFourthMeld::Minjun(t, ShunWait::Kanchan | ShunWait::Penchan | ShunWait::RyanmenLow | ShunWait::RyanmenHigh) => Some(t),

			ScorableHandFourthMeld::Minkan(..) |
			ScorableHandFourthMeld::Minkou(_, KouWait::Tanki) |
			ScorableHandFourthMeld::Minjun(_, ShunWait::Tanki) => return 0,
		};
		if let Some(t) = t {
			let offset = t.remove_red() as u8;
	...
	fn chanta_routou(&self) -> ChantaRoutou {
		self.melds().fold(ChantaRoutou(0), |acc, m| acc | m.chanta_routou()) | self.pair.chanta_routou()
	}

	fn is_sanshoku(&self, mut f: impl FnMut(ScorableHandMeld) -> Option<NumberTile>) -> bool {
		const MASK: u64 = 0b000000001_000000001_000000001_000000001_000000001_000000001_u64;
		...
		for m in self.melds() {
			...
			let mask = ((MASK << offset) >> 27) as u32;
			if counts & mask == mask & 0x7FFFFFF {
				return true;
			}
		}
	...
	fn num_ankou(&self) -> NumAnkou {
		let count = self.melds().filter(|m| matches!(m, ScorableHandMeld::Ankan(_) | ScorableHandMeld::Ankou(_))).count();
		match count {
			3 => NumAnkou::Sanankou,
			4 => NumAnkou::Suuankou { is_tanki_wait: self.m4.is_tanki() },
			_ => NumAnkou::Neither,
		}
	}
	...
	pub(crate) fn is_pinfu(&self, round_wind: WindTile, seat_wind: WindTile) -> bool {
		matches!(
			...
			Self {
				...
				pair,
			} if pair.num_yakuhai(round_wind, seat_wind) == 0,
		)
	}
	...
	fn is_tanyao(&self) -> bool {
		...
		self.melds().fold(true, |acc, m| acc & m.is_tanyao()) & self.pair.is_tanyao()
	}
	...
	pub(crate) fn is_dragon_yakuhai(&self, dragon: DragonTile) -> bool {
		...
		self.melds().fold(false, |acc, m| acc | m.is_dragon_yakuhai(dragon))
	}
	...
	pub(crate) fn is_toitoi(&self) -> bool {
		// Micro-optimization: Using `.all(matches!())` generates branches due to the short-circuiting.
		self.melds().fold(true, |acc, m| acc & matches!(
			m,
			ScorableHandMeld::Ankan(_) |
			ScorableHandMeld::Minkan(_) |
			ScorableHandMeld::Ankou(_) |
	...
	pub(crate) fn num_chuuren_poutou(&self) -> u8 {
		...
		fn fourth_meld_and_pair_old_and_new_tiles(m4: ScorableHandFourthMeld, pair: ScorableHandPair, suit_expected: NumberSuit) -> Option<(Number, Number, Number, Number, Number)> {
			...
			match m4 {
				...
				ScorableHandFourthMeld::Minkan(..) |
				ScorableHandFourthMeld::Minkou(_, KouWait::Tanki) |
				ScorableHandFourthMeld::Minjun(_, ShunWait::Tanki) => return None,

				ScorableHandFourthMeld::Ankou(t, KouWait::Tanki) => {
					{
						let t = NumberTile::try_from(t).ok()?;
				...
				ScorableHandFourthMeld::Minkou(t, KouWait::Shanpon) => {
					{
						let t = NumberTile::try_from(t).ok()?;
						let number = t.is_in_suit(suit_expected)?;
						old1 = number;
...
impl ScorableHandChiitoi {
	...
	fn chanta_routou(self) -> ChantaRoutou {
		self.0.into_iter().fold(ChantaRoutou(0), |acc, p| acc | p.chanta_routou())
	}
...
impl const From<ScorableHandFourthMeld> for ScorableHandMeld {
	fn from(meld: ScorableHandFourthMeld) -> Self {
		match meld {
			...
			ScorableHandFourthMeld::Minkan(t, _) => Self::Minkan(t),
			ScorableHandFourthMeld::Ankou(t, _) => Self::Ankou(t),
			ScorableHandFourthMeld::Minkou(t, _) => Self::Minkou(t),
			ScorableHandFourthMeld::Anjun(t, _) => Self::Anjun(t),
			ScorableHandFourthMeld::Minjun(t, _) => Self::Minjun(t),
		}
	}
}
...
impl const Ord for ScorableHandMeld {
	fn cmp(&self, other: &Self) -> core::cmp::Ordering {
		const fn sort_criteria(m: ScorableHandMeld) -> u16 {
			...
			let sc = unsafe { core::mem::transmute::<ScorableHandMeld, [u8; 2]>(m) };
			u16::from_le_bytes(sc)
		}
...
impl ScorableHandFourthMeld {
	...
	fn for_each_tile(self, f: impl FnMut(Tile)) {
		ScorableHandMeld::from(self).for_each_tile(f);
	}
	...
	fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
		...
		if is_ok {
			f.write_str(match self {
				...
				Self::Minkou(_, KouWait::Tanki) |
				Self::Anjun(_, ShunWait::Tanki) |
				Self::Minjun(_, ShunWait::Tanki) => "}",
				Self::Ankou(_, KouWait::Shanpon) |
				Self::Minkou(_, KouWait::Shanpon) => "shanpon }",
				Self::Anjun(_, ShunWait::Kanchan) |
				Self::Minjun(_, ShunWait::Kanchan) => "kanchan }",
				Self::Anjun(_, ShunWait::Penchan) |
				Self::Minjun(_, ShunWait::Penchan) => "penchan }",
				Self::Anjun(_, ShunWait::RyanmenLow) |
				Self::Minjun(_, ShunWait::RyanmenLow) => "ryanmen_low }",
				Self::Anjun(_, ShunWait::RyanmenHigh) |
				Self::Minjun(_, ShunWait::RyanmenHigh) => "ryanmen_high }",
			})
		}
		else {
			Err(core::fmt::Error)
...
impl const Ord for ScorableHandFourthMeld {
	fn cmp(&self, other: &Self) -> core::cmp::Ordering {
		const fn sort_criteria(m: ScorableHandFourthMeld) -> u32 {
			...
			let sc = unsafe { core::mem::transmute_copy::<ScorableHandFourthMeld, [core::mem::MaybeUninit<u8>; core::mem::size_of::<ScorableHandFourthMeld>()]>(&m) };

			// sc is `[d, t, w, padding]`. Transform it to `[w, zero, d, t]` and then to `t:d:zero:w`.
			//
			...
			// `[d, t, w, padding]` -> `[zero, d, t, w]`
			//
			// Note that rustc stores `[MU<u8>; 4]` in a register like a `u32`, and is smart enough to notice that this is just `<< 8` on that `u32`.
			//
			// SAFETY: `sc[0..=2]` have defined values.
			let sc = unsafe { [0, sc[0].assume_init(), sc[1].assume_init(), sc[2].assume_init()] };
