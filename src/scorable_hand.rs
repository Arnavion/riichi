...
impl ScorableHandRegular {
	...
	pub(crate) fn chanta_routou(&self) -> ChantaRoutou {
		...
		const CHANTA_ROUTOU_OTHER: core::simd::Simd<u8, 5> = core::simd::Simd::splat(ChantaRoutou::other().0);
		const CHANTA_ROUTOU_HAS_TERMINALS: core::simd::Simd<u8, 5> = core::simd::Simd::splat(ChantaRoutou::has_terminals().0);
		const CHANTA_ROUTOU_ALL_HONORS: core::simd::Simd<u8, 5> = core::simd::Simd::splat(ChantaRoutou::all_honors().0);
		const CHANTA_ROUTOU_ALL_TERMINALS: core::simd::Simd<u8, 5> = core::simd::Simd::splat(ChantaRoutou::all_terminals().0);
	...
	pub(crate) fn is_uumensai(&self) -> bool {
		...
		let is_pin_or_sou_or_wind_or_dragon = core::simd::cmp::SimdPartialOrd::simd_ge(ts, core::simd::Simd::splat(t!(1p) as u8));
		let is_sou_or_wind_or_dragon = core::simd::cmp::SimdPartialOrd::simd_ge(ts, core::simd::Simd::splat(t!(1s) as u8));
		let is_wind_or_dragon = core::simd::cmp::SimdPartialOrd::simd_ge(ts, core::simd::Simd::splat(t!(E) as u8));
		let is_dragon = core::simd::cmp::SimdPartialOrd::simd_ge(ts, core::simd::Simd::splat(t!(Wh) as u8));
		let suits =
			core::simd::Select::select(is_pin_or_sou_or_wind_or_dragon, ONES, ZEROS) +
			core::simd::Select::select(is_sou_or_wind_or_dragon, ONES, ZEROS) +
			core::simd::Select::select(is_wind_or_dragon, ONES, ZEROS) +
			core::simd::Select::select(is_dragon, ONES, ZEROS);
	...
	pub(crate) fn is_ryuuiisou(&self) -> bool {
		...
		let sets = core::simd::Select::select(is_kan_kou_shun, KOU_KAN_PAIR_VALID, SHUN_VALID);
		let offsets = ts >> 1;
		let masks = core::simd::Simd::splat(0b1) << core::simd::num::SimdUint::cast::<u64>(offsets);
...
impl ScorableHandChiitoi {
	...
	pub(crate) fn chanta_routou(self) -> ChantaRoutou {
		const CHANTA_ROUTOU_OTHER: core::simd::Simd<u8, 7> = core::simd::Simd::splat(ChantaRoutou::other().0);
		const CHANTA_ROUTOU_ALL_HONORS: core::simd::Simd<u8, 7> = core::simd::Simd::splat(ChantaRoutou::all_honors().0);
		const CHANTA_ROUTOU_ALL_TERMINALS: core::simd::Simd<u8, 7> = core::simd::Simd::splat(ChantaRoutou::all_terminals().0);
