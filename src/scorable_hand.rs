...
impl ScorableHandPair {
	...
	pub(crate) fn num_yakuhai(self, round_wind: WindTile, seat_wind: WindTile) -> u8 {
		...
		//
		// The pats line up with the explicit values given to the `Tile` variants, and tested exhaustively in the `num_yakuhai` test.
		match self.0[0] as u8 {
			// NumberTile
			..30 => 0,
			// WindTile
			t @ 30..34 => u8::from(round_wind as u8 == t) + u8::from(seat_wind as u8 == t),
			// DragonTile
			34.. => 1,
		}
	}
...
impl core::ops::Add for MeldChantaRoutou {
...
impl core::ops::Add<MeldChantaRoutou> for HandChantaRoutou {
	type Output = Self;
