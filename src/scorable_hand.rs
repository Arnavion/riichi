...
/// If this expectation is violated, the program may have undefined behavior.
#[derive(Clone, Copy)]
#[repr(C, u8, align(2))] // See comment in `ScorableHandMeld::cmp`.
...
impl ScorableHandRegular {
	pub fn new(ma: ScorableHandMeld, mb: ScorableHandMeld, mc: ScorableHandMeld, md: ScorableHandFourthMeld, pair: ScorableHandPair) -> Self {
		...
		Self::new_already_sorted(m1, m2, m3, m4, pair)
	}

	pub(crate) const fn new_already_sorted(m1: ScorableHandMeld, m2: ScorableHandMeld, m3: ScorableHandMeld, m4: ScorableHandFourthMeld, pair: ScorableHandPair) -> Self {
	...
	pub(crate) fn peikou_isshoku_jun(&self) -> PeikouIsshokuJun {
		let Self { m1, m2, m3, m4, .. } = self;
		PeikouIsshokuJun::new(*m1, *m2, *m3, *m4, self.is_menzen())
	}
	...
	pub(crate) fn chanta_routou(&self) -> ChantaRoutou {
		cfg_select! {
			all(target_arch = "riscv64", target_feature = "v") => {
				...
				const KAN_KOU_TERMINALS: Tile34Set = t34set![1m, 9m, 1p, 9p, 1s, 9s];
				const KAN_KOU_HONORS: Tile34Set = t34set![E, S, W, G, N, Wh, G, R];
				...
				unsafe {
					core::arch::asm!(
						...
						// let offsets /* : v5:v5 */ = Tile::offsets(ts);
						"vsrl.vi v5, v5, 1",
						// let offsets /* : v16:v19 */ = offsets.cast::<u64>();
						"vsetvli zero, zero, e64, m4, ta, ma",
						"vzext.vf8 v16, v5",
						// let masks /* : v8:v11 */ = 1 << offsets;
						"vmv.v.i v8, 1",
						"vsll.vv v8, v8, v16",

						// let shun_terminals_contains_t /* : v0:v0 */ = (masks & SHUN_TERMINALS.present) != 0;
						...
						// let kan_kou_honors_contains_t /* : v0:v0 */ = (masks & KAN_KOU_HONORS.present) != 0;
						"vsetvli zero, zero, e64, m4, ta, ma",
						"vand.vx v16, v8, {kan_kou_honors}",
						"vmsne.vi v0, v16, 0",
						// let temp2 /* : v12:v12 */ = select(kan_kou_honors_contains_t, chanta_routou_all_honors, chanta_routou_other);
						"vsetvli zero, zero, e8, m1, ta, ma",
						"vmerge.vim v12, v12, {chanta_routou_all_honors}, v0",

						// let kan_kou_terminals_contains_t /* : v0:v0 */ = (masks & KAN_KOU_TERMINALS.present) != 0;
						"vsetvli zero, zero, e64, m4, ta, ma",
						"vand.vx v16, v8, {kan_kou_terminals}",
						"vmsne.vi v0, v16, 0",
						// let temp3 /* : v12:v12 */ = select(kan_kou_terminals_contains_t, chanta_routou_all_terminals, temp2);
						"vsetvli zero, zero, e8, m1, ta, ma",
						...
						kan_kou_terminals = in(reg) KAN_KOU_TERMINALS.present,
						kan_kou_honors = in(reg) KAN_KOU_HONORS.present,
	...
	pub(crate) fn num_wind_yakuhai(&self, round_wind: WindTile, seat_wind: WindTile) -> [u8; 4] {
		...
		for wind in [round_wind, seat_wind] {
			...
			result[(wind as usize - tw!(E) as usize) >> 1] += u8::from(is_wind);
		}
		result
	}

	pub(crate) fn is_dragon_yakuhai(&self) -> [bool; 3] {
		td![Wh, G, R].map(|dragon| {
			// Micro-optimization: `.contains()` generates a branch for each element test.
			#[expect(clippy::manual_contains)]
	...
	pub(crate) fn is_shiiaru_raotai(&self) -> bool {
		let is_fully_open = self.melds().iter().all(|m| matches!(m, ScorableHandMeld::Ankan(_) | ScorableHandMeld::Minkan(_) | ScorableHandMeld::Minkou(_) | ScorableHandMeld::Minjun(_)));
		is_fully_open & self.m4.is_tanki()
	}

	pub(crate) fn iisou(&self) -> Iisou {
		Iisou::new(self.melds_and_pair())
	}
	...
	pub(crate) fn ittsuukansen(&self) -> IttsuuKanSen {
		IttsuuKanSen::new(self.melds_and_pair(), self.is_menzen())
	}
	...
	pub(crate) fn num_ankou(&self) -> NumAnkou {
		NumAnkou::new(self.melds_types(), self.m4.is_tanki())
	}
	...
	pub(crate) fn num_kantsu(&self) -> NumKantsu {
		NumKantsu::new(self.melds_types())
	}

	pub(crate) fn suushii_sangen(&self) -> SuushiiSangen {
		SuushiiSangen::new(self.melds_tiles(), self.pair.inner.0)
	}
	...
	pub(crate) fn is_akadora_sanshoku(&self) -> bool {
		is_akadora_sanshoku(&self.melds_and_pair_tiles())
	}
	...
	pub(crate) fn num_chuuren_poutou(&self) -> u8 {
		...
		fn fourth_meld_and_pair_old_and_new_tiles(m4: ScorableHandFourthMeld, pair: ScorableHandPair, suit_expected: NumberSuit) -> Option<(Number, Number, Number, Number, Number)> {
			...
			match m4.dw() {
				...
				ScorableHandFourthMeldDw::MinjunTanki => return None,

				ScorableHandFourthMeldDw::AnkouTanki => {
				...
				ScorableHandFourthMeldDw::AnjunTanki => {
				...
				ScorableHandFourthMeldDw::MinkouShanpon => {
					let t = unsafe { core::mem::transmute::<u8, Tile>(m4_t) };
				...
				ScorableHandFourthMeldDw::MinjunKanchan => {
					let t = unsafe { core::mem::transmute::<u8, ShunLowTileAndHasFiveRed>(m4_t) };
				...
				ScorableHandFourthMeldDw::MinjunPenchan => {
					let t = unsafe { core::mem::transmute::<u8, ShunLowTileAndHasFiveRed>(m4_t) };
				...
				ScorableHandFourthMeldDw::MinjunRyanmenLow => {
					let t = unsafe { core::mem::transmute::<u8, ShunLowTileAndHasFiveRed>(m4_t) };
				...
				ScorableHandFourthMeldDw::MinjunRyanmenHigh => {
					let t = unsafe { core::mem::transmute::<u8, ShunLowTileAndHasFiveRed>(m4_t) };
	...
	pub(crate) fn is_hyakuman_goku(&self) -> bool {
		let valid = self.melds_and_pair_tiles().iter().all(|&t| t <= t!(9m) as u8);
		if !valid {
			return false;
		}

		let pair_value = (Tile::offset(self.pair.inner.0) + 1) * 2;
		let values =
			self.melds().iter()
			.fold(pair_value, |acc, m| acc + match m {
				ScorableHandMeld::Ankan(t) | ScorableHandMeld::Minkan(t) => (Tile::offset(*t) + 1) * 4,
				ScorableHandMeld::Ankou(t) | ScorableHandMeld::Minkou(t) => (Tile::offset(*t) + 1) * 3,
				ScorableHandMeld::Anjun(t) | ScorableHandMeld::Minjun(t) => (Tile::offset(t.remove_red().into()) + 2) * 3,
			});
		values >= 100
	}
	...
	pub(crate) fn is_golden_gate_bridge(&self) -> bool {
		...
		let counts =
			self.melds_and_pair().iter().skip(1)
			.fold(0b000000000_000000000_000000000_u32, |counts, m| {
				let [d, t] = m.parts();
				counts | u32::from(d >= 4).wrapping_shl(((t - t!(1m) as u8) >> 1).into())
			});
		let is_golden_gate_bridge = [counts, counts >> 9, counts >> 18].iter().any(|&counts| counts & MASK == MASK);
		self.is_menzen() && is_golden_gate_bridge
	}
	...
	fn is_sanshoku(&self, f: impl FnMut(&ScorableHandMeld) -> Option<NumberTile>) -> bool {
		...
		self.melds().iter()
			.filter_map(f)
			.fold(false, |result, t| {
				let offset = Tile::offset(t.into());

				counts |= Tile::mask::<u32>(offset);
...
impl ScorableHandChiitoi {
	...
	pub(crate) fn is_toipuutao(self) -> bool {
		self.0.iter().all(|p| Tile34Set::REVERSIBLE.contains(p.0))
	}
			...
			all(target_arch = "riscv64", target_feature = "v") => {
				...
				unsafe {
					core::arch::asm!(
						...
						// let honors_contains_t /* : v0:v0 */ = (masks & HONORS.present) != 0;
						...
						// let terminals_contains_t /* : v0:v0 */ = (masks & TERMINALS.present) != 0;
						"vsetvli zero, zero, e64, m4, ta, ma",
	...
	pub(crate) fn is_uumensai(self) -> bool {
		let mut mask = 0b00000;
		for &ScorableHandPair(t) in &self.0 {
			let suit = 4 - u8::from(t < t!(1p)) - u8::from(t < t!(1s)) - u8::from(t < t!(E)) - u8::from(t < t!(Wh));
			mask |= 0b1 << suit;
		}
		mask == 0b11111
	}
	...
	pub(crate) fn is_akadora_sanshoku(self) -> bool {
		is_akadora_sanshoku(&self.tiles())
	}
	...
	pub(crate) fn dairin_kokuiisou(self) -> DairinKokuiisou {
		DairinKokuiisou::new(self.0)
	}

	fn tiles(self) -> [u8; 7] {
		self.0.map(|ScorableHandPair(t)| t as u8)
	}
}
...
impl core::fmt::Display for ScorableHandKokushiMusou {
	fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
		...
		let offset = Tile::offset(self.duplicate);
...
impl ScorableHandMeld {
	...
	fn for_each_tile(self, mut f: impl FnMut(Tile)) {
		// TODO(rustup): Use `!` when that is stabilized.
		#[expect(clippy::unit_arg)]
		self.try_for_each_tile::<core::convert::Infallible>(|t| Ok(f(t)));
	}

	fn try_for_each_tile<E>(self, mut f: impl FnMut(Tile) -> Result<(), E>) -> Result<(), E> {
		match self {
			Self::Ankan(t1) |
			Self::Minkan(t1) => {
...
impl core::fmt::Display for ScorableHandMeld {
	fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
		...
		f.write_str(s)?;
		self.try_for_each_tile(|t| write!(f, "{t} "))?;
		f.write_str("}")
	}
}
...
impl Ord for ScorableHandMeld {
	fn cmp(&self, other: &Self) -> core::cmp::Ordering {
		const fn sort_criteria(m: ScorableHandMeld) -> u16 {
...
impl PartialEq for ScorableHandMeld {
	fn eq(&self, other: &Self) -> bool {
		...
		// mentioned in `ScorableHandMeld::cmp` above.
...
impl core::fmt::Display for ScorableHandFourthMeld {
	fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
		...
		self.as_ref().try_for_each_tile(|t| write!(f, "{t} "))?;
...
impl Ord for ScorableHandFourthMeld {
	fn cmp(&self, other: &Self) -> core::cmp::Ordering {
		const fn sort_criteria(m: ScorableHandFourthMeld) -> u32 {
			...
			// Construct `[w, zero, d, t]` and then convert it to `t:d:zero:w`. Comparing this integer produces the required sort order.
			//
			// `zero:t:d:w` or `t:d:w:zero` would also work but those arrangements require more instructions to produce.
			let sc = [w, 0, d, t];
			u32::from_le_bytes(sc)
		}
...
impl PartialEq for ScorableHandFourthMeld {
	fn eq(&self, other: &Self) -> bool {
		// Micro-optimization: If we do `matches!(self.cmp(other), Ordering::Equal)`, rustc inlines `Ord::cmp` but doesn't elide the rotation,
		// even though that won't affect the equality comparison. So we compare the parts ourselves instead of delegating to `Ord::cmp`.
...
impl PeikouIsshokuJun {
	fn new(m1: ScorableHandMeld, m2: ScorableHandMeld, m3: ScorableHandMeld, m4: ScorableHandFourthMeld, is_menzen: bool) -> Self {
...

		{
			...
			let offset = (m1_t & !0b1) - t!(1m) as u8;

			peikou_counts -= consider_m1.wrapping_shl(offset.into());

			isshoku_jun_counts -= consider_m1.wrapping_shl(offset.into());
		}

		{
			let consider_m2 = u64::from(m2_d >= 4);
			let offset = (m2_t & !0b1) - t!(1m) as u8;
...
impl Iisou {
	fn new(melds_and_pair: &[ScorableHandMeld; 5]) -> Self {
		#[expect(clippy::needless_pass_by_value)]
		fn new_inner(
			melds_and_pair: &[ScorableHandMeld; 5],
			...
			melds_and_pair.iter()
				.fold(true, |is_valid, &m| {
					let (set, t) = match m {
						...
						ScorableHandMeld::Minkan(t) |
						ScorableHandMeld::Ankou(t) |
						ScorableHandMeld::Minkou(t) => (&kan_kou_pair_valid, t),
						ScorableHandMeld::Anjun(t) |
		...
		if new_inner(melds_and_pair, Tile34Set::REVERSIBLE, t34set![1p, 2p, 3p, 4s]) {
			Self::Toipuutao
		}
		else if new_inner(melds_and_pair, t34set![2s, 3s, 4s, 6s, 8s], t34set![2s]) {
			Self::Chinryuusou
		}
		else if new_inner(melds_and_pair, t34set![2s, 3s, 4s, 6s, 8s, G], t34set![2s]) {
			// Ryuuiisou requires G, but a hand without G would've already matched as Chinryuusou.
			Self::Ryuuiisou
		}
		else if new_inner(melds_and_pair, t34set![2s, 3s, 4s, 6s, 8s, R], t34set![2s]) {
			// Kouitten requires R, but a hand without R would've already matched as Chinryuusou.
			Self::Kouitten
		}
		else if new_inner(melds_and_pair, t34set![1s, 5s, 7s, 9s, R], Default::default()) {
			Self::Benikujaku
		}
		else if new_inner(melds_and_pair, Tile34Set::BLACK, Default::default()) {
			Self::Kokuiisou
		}
		else {
			Self::None
...
impl IttsuuKanSen {
	fn new(melds_and_pair: &[ScorableHandMeld; 5], is_menzen: bool) -> Self {
		const SANSHOKU_TSUUKAN_MASKS: [u32; 6] = [
		...
		let counts =
			melds_and_pair.iter().skip(1)
			.fold(0b000000000_000000000_000000000_u32, |counts, m| {
				let [d, t] = m.parts();
				counts | u32::from(d >= 4).wrapping_shl(((t - t!(1m) as u8) >> 1).into())
			});

		// Wrong lint. Ref: https://github.com/rust-lang/rust-clippy/issues/17563
		#[expect(clippy::manual_contains)]
		let is_sanshoku_tsuukan = SANSHOKU_TSUUKAN_MASKS.iter().any(|&mask| counts & mask == mask);
		if is_sanshoku_tsuukan {
			return Self::SanshokuTsuukan;
		}

		let is_ittsuu = [counts, counts >> 9, counts >> 18].iter().any(|&counts| counts & ITTSUU_MASK == ITTSUU_MASK);
		let result = u8::from(is_ittsuu) << 1;

		let counts =
			melds_and_pair.iter()
			.fold(0b0000000_000000000_000000000_000000000_u64, |counts, m| {
				let [_, t] = m.parts();
				counts | 0b1_u64.wrapping_shl(((t - t!(1m) as u8) >> 1).into())
			});
		let is_touhoku_shinkansen = is_menzen && (counts & TOUHOKU_SHINKANSEN_WINDS.present) == TOUHOKU_SHINKANSEN_WINDS.present;
...
impl NumAnkou {
	fn new(ds: [u8; 4], m4_is_tanki: bool) -> Self {
		let count = ds.iter().filter(|&&d| d & 0b101 == 0b000).count();
...
impl NumKantsu {
	fn new(ds: [u8; 4]) -> Self {
		let count = ds.iter().filter(|&&d| d <= 1).count();
		match count {
			3 => Self::Sankantsu,
			4 => Self::Suukantsu,
			_ => Self::Neither,
...
impl NumRenkou {
	fn new(h: &ScorableHandRegular) -> Self {
		const SUURENKOU_MASK: u64 = 0b1111;
		const SANRENKOU_MASK: u64 = 0b111;

		let masks = h.melds().iter().fold(0b000000000_000000000_000000000_u64, |masks, m| {
			let [d, t] = m.parts();
			masks | (u64::from(d <= 3) << ((t - t!(1m) as u8) >> 1))
		});
		let man_counts = masks & 0b111111111;
		let pin_counts = masks & (0b111111111 << 9);
		let sou_counts = masks & (0b111111111 << 18);

		let is_suurenkou = h.melds_tiles().iter().any(|&t| {
			let expected = SUURENKOU_MASK << ((t - t!(1m) as u8) >> 1);
		...
		let is_sanrenkou = h.melds_tiles().iter().any(|&t| {
			let expected = SANRENKOU_MASK << ((t - t!(1m) as u8) >> 1);
			let actual_man = man_counts & expected;
			let actual_pin = pin_counts & expected;
			let actual_sou = sou_counts & expected;
			actual_man == expected || actual_pin == expected || actual_sou == expected
...
fn is_akadora_sanshoku(ts: &[u8]) -> bool {
	ts.iter().map(|&t| t & 0b1).sum::<u8>() == 3
}
...
impl DairinKokuiisou {
	fn new(ts: [ScorableHandPair; 7]) -> Self {
		const DAICHIISHIN_VALID: u64 = Tile34Set::HONORS.present;
		const KOKUIISOU_VALID: u64 = Tile34Set::BLACK.present;

		let masks = ts.iter().fold(0b0000000_000000000_000000000_000000000_u64, |acc, ScorableHandPair(t)| acc | Tile::mask::<u64>(Tile::offset(*t)));
		match masks {
			0b0000000_000000000_000000000_011111110 => Self::Daisuurin,
			0b0000000_000000000_011111110_000000000 => Self::Daisharin,
			0b0000000_011111110_000000000_000000000 => Self::Daichikurin,
			DAICHIISHIN_VALID => Self::Daichiishin,
			KOKUIISOU_VALID => Self::Kokuiisou,
			_ => Self::None,
		}
	}
}
...
mod tests {
	...
	macro_rules! test {
		...
		(@inner_new_tile $hand:ident + $new_tile:tt => [ $( $scorable_hand:tt => { $($funcs:tt)* } )* ] $($rest:tt)*) => {{
			{
				...
				#[allow(unused_mut)]
				let mut hands: std::collections::BTreeSet<_> = $hand.clone().to_scorable_hands(t!($new_tile), $crate::TsumoOrRon::Tsumo).collect();
				$({
					let hand = hands.pop_first().unwrap();
					assert_eq!(hand, make_scorable_hand! $scorable_hand);
					test!(@inner_func hand $($funcs)*);
				})*
				assert!(hands.is_empty(), "{hands:?}");
			}
		...
		(@inner_new_tile $hand:ident + $new_tile:tt ron => [ $( $scorable_hand:tt => { $($funcs:tt)* } )* ] $($rest:tt)*) => {{
			{
				std::println!("hand: {:?} + {}", $hand, t!($new_tile));
				#[allow(unused_mut)]
				let mut hands: std::collections::BTreeSet<_> = $hand.clone().to_scorable_hands(t!($new_tile), $crate::TsumoOrRon::Ron).collect();
				$({
					let hand = hands.pop_first().unwrap();
					assert_eq!(hand, make_scorable_hand! $scorable_hand);
					test!(@inner_func hand $($funcs)*);
		...
		($hand:tt $($new_tile:tt)*) => {{
			let hand = $crate::HandStable::from($crate::make_hand! $hand);
	...
	fn chinitsu1() {
		test!((1p 2p 3p 3p 4p 5p 0p 6p 6p 7p 9p 9p 9p)
			+ 3p => [
				...
				({ anjun 1p 2p 3p } { anjun 4p 0p 6p } { anjun 5p 6p 7p } { ankou 9p 9p 9p } { 3p 3p }) => {
					is_chinitsu();
				}
				({ anjun 4p 5p 6p } { anjun 0p 6p 7p } { ankou 9p 9p 9p } { anjun 1p 2p 3p penchan } { 3p 3p }) => {
					is_chinitsu();
				}
				({ anjun 4p 0p 6p } { anjun 5p 6p 7p } { ankou 9p 9p 9p } { anjun 1p 2p 3p penchan } { 3p 3p }) => {
					is_chinitsu();
				}
			]
			+ 6p => [
				...
				({ anjun 1p 2p 3p } { anjun 3p 4p 0p } { anjun 5p 6p 7p } { ankou 9p 9p 9p } { 6p 6p }) => {
					is_chinitsu();
				}
				({ anjun 1p 2p 3p } { anjun 3p 4p 0p } { ankou 9p 9p 9p } { anjun 5p 6p 7p kanchan } { 6p 6p }) => {
					is_chinitsu();
				}
			]
		);
	...
	fn daichikurin1() {
		test!((2s 2s 3s 4s 4s 0s 5s 6s 6s 7s 7s 8s 8s)
			+ 3s => [
				...
				({ anjun 3s 4s 0s } { anjun 6s 7s 8s } { anjun 6s 7s 8s } { anjun 3s 4s 5s ryanmen_low } { 2s 2s }) => {
					is_tanyao();
					is_ryanpeikou();
					is_chinitsu();
				}
