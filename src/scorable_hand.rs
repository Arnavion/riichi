			...
			all(target_arch = "riscv64", target_feature = "v") => {
				...
				unsafe {
					core::arch::asm!(
						...
						// let kou_kan_honors_contains_t /* : v0:v0 */ = (masks & KOU_KAN_HONORS.present) != 0;
						"vsetvli zero, zero, e64, m4, ta, ma",
						...
						// let kou_kan_terminals_contains_t /* : v0:v0 */ = (masks & KOU_KAN_TERMINALS.present) != 0;
						"vsetvli zero, zero, e64, m4, ta, ma",
...
impl ScorableHandChiitoi {
	...
	pub(crate) fn dairin(self) -> Dairin {
		let masks = self.0.iter().fold(0b0000000_000000000_000000000_000000000_u64, |acc, ScorableHandPair(t)| acc | 0b1_u64 << ((*t as u8 - t!(1m) as u8) >> 1));
		match masks {
	...
	pub(crate) fn is_kokuiisou(self) -> bool {
		const VALID: Tile34Set = t34set! { 2p, 4p, 8p, E, S, W, N };
...
impl ScorableHandFourthMeld {
	...
	/// ===+===+===========+=====================
	///  4 | 1 | 8         | Anjun kanchan
	///  5 | 1 | 9         | Minjun kanchan
	/// ===+===+===========+=====================
	///  4 | 2 | 12        | Anjun penchan
	///  5 | 2 | 13        | Minjun penchan
	/// ===+===+===========+=====================
...
mod tests {
	...
	impl ScorableHand {
		...
		fn is_daisharin(&self) -> bool {
			match self {
				Self::Chiitoi(h) => matches!(h.dairin(), Dairin::Pin),
				Self::Regular(_) |
				Self::KokushiMusou(_) => false,
			}
		}
		...
		fn is_daichikurin(&self) -> bool {
			match self {
				Self::Chiitoi(h) => matches!(h.dairin(), Dairin::Sou),
				Self::Regular(_) |
				Self::KokushiMusou(_) => false,
			}
		}
		...
		fn is_daisuurin(&self) -> bool {
			match self {
				Self::Chiitoi(h) => matches!(h.dairin(), Dairin::Man),
				Self::Regular(_) |
				Self::KokushiMusou(_) => false,
			}
		}
		...
		fn is_daichisei(&self) -> bool {
			match self {
				Self::Chiitoi(h) => matches!(h.dairin(), Dairin::Ji),
				Self::Regular(_) |
				Self::KokushiMusou(_) => false,
			}
		}
	...
	macro_rules! test {
		...
		(@inner_func $hand:ident $($rest:tt)*) => {
			test! {
				...
				{ assert!(!$hand.is_daichisei(), "!is_daichisei"); }
	...
	// Ref: https://riichi.wiki/index.php?title=Chanta&oldid=30713
	#[test]
	fn chanta2() {
	...
	// Ref: https://riichi.wiki/index.php?title=Suushiihou&oldid=30642
	#[test]
	fn suushii3() {
