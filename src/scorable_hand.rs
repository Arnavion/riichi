...
impl ScorableHand {
	...
	pub(crate) fn honchinitsu(&self) -> Honchinitsu {
		fn inner(result: &mut u8, suit: &mut Option<NumberSuit>, t: Tile) {
			...
			Self::Regular(h @ ScorableHandRegular { pair, .. }) => {
				for m in h.melds() {
					let t = match m {
						...
						ScorableHandMeld::Minkou(t) => t,
						ScorableHandMeld::Anjun(t) |
						ScorableHandMeld::Minjun(t) => t.into(),
					};
					inner(&mut result, &mut suit, t);
				}
				inner(&mut result, &mut suit, pair.0);
			},
...
impl ScorableHandRegular {
	pub fn new(mut mabc: [ScorableHandMeld; 3], md: ScorableHandFourthMeld, pair: ScorableHandPair) -> Self {
		let melds = match md {
	...
	pub(crate) fn num_peikou(&self) -> usize {
		...
		for m in [m2, m3] {
			match m {
				...
				ScorableHandMeld::Anjun(t) => {
					...
					//   0b10 | 0b01
					//
					// We could do `counts = (counts ^ (0b1 << offset)) & !(0b1 << (offset + 1))`, which on RV would be three instructions
					// `binv counts, counts, offset; ori offset, offset, 1; bclr counts, counts, offset`. We can do better with
					// `counts -= 0b1 << offset;`, which on RV would be just two instructions
					...
					// `li temp, -1; sll temp, temp, offset; add counts, counts, temp`. As usual, wrapping `0b1 << offset` in `core::hint::black_box()`
					// makes rustc emit the expected sequence, but inserts a stack store and load of the black-boxed value.
					//
		...
		let t = match m4 {
			...
			ScorableHandFourthMeld::Tanki(ScorableHandMeld::Anjun(t)) => Some(t.0),

			ScorableHandFourthMeld::Kanchan { tile, .. } |
			ScorableHandFourthMeld::Penchan { tile, .. } |
			ScorableHandFourthMeld::RyanmenLow { tile, .. } |
			ScorableHandFourthMeld::RyanmenHigh { tile, .. } => Some(tile.0),
	...
	pub(crate) fn suushii_sangen(&self) -> SuushiiSangen {
		...
		for m in self.melds() {
			...
			num_wind_melds += u8::from(WindTile::try_from(t).is_ok());
			num_dragon_melds += u8::from(DragonTile::try_from(t).is_ok());
		}
	...
	pub(crate) fn is_ryuuiisou(&self) -> bool {
		...
		let kou_kan_pair_valid = t34set! { 2s, 3s, 4s, 6s, 8s, G };
		let shun_valid = t34set! { 2s };
		...
		for m in self.melds() {
			let (set, t) = match m {
				...
				ScorableHandMeld::Anjun(t) |
				ScorableHandMeld::Minjun(t) => (&shun_valid, Tile::from(t)),
			};
...
impl ScorableHandMeld {
	...
	pub fn ankan(t1: Tile, t2: Tile, t3: Tile, t4: Tile) -> Self {
	...
	pub fn minkan(t1: Tile, t2: Tile, t3: Tile, t4: Tile) -> Self {
		debug_assert!([t1, t2, t3].eq_ignore_red(&[t2, t3, t4]));
	...
	pub fn ankou(t1: Tile, t2: Tile, t3: Tile) -> Self {
	...
	pub fn minkou(t1: Tile, t2: Tile, t3: Tile) -> Self {
		debug_assert!([t1, t2].eq_ignore_red(&[t2, t3]));
	...
	pub fn anjun(t1: ShunLowTile, t2: NumberTile, t3: NumberTile) -> Self {
	...
	pub fn minjun(t1: ShunLowTile, t2: NumberTile, t3: NumberTile) -> Self {
		let t = ShunLowTileAndHasFiveRed::new(t1, t2, t3);
		Self::Minjun(t)
	}
	...
	fn is_tanyao(self) -> bool {
		...
		match self {
			...
			Self::Anjun(t) |
			Self::Minjun(t) => shun_simples.contains(NumberTile::const_from_slthfr(t)),
		}
	}

	fn chanta_routou(self) -> ChantaRoutou {
		let shun_terminals = t34set! { 1m, 7m, 1p, 7p, 1s, 7s };
		let kou_kan_terminals = t34set! { 1m, 9m, 1p, 9p, 1s, 9s };
		let kou_kan_honors = t34set! { E, S, W, G, N, Wh, G, R };
		...
		let (t, is_shun) = match self {
			...
			Self::Anjun(t) |
			Self::Minjun(t) => (Tile::const_from_slthfr(t), true),
		};
...
impl ScorableHandMeldSortCriteria {
	// Taking `ScorableHandMeld` as clippy recommends makes it more complicated to use this fn point-free with `slice.sort_unstable_by_key()`.
	...
	pub(crate) const fn new(m: &ScorableHandMeld) -> Self {
		...
		let t = match m {
			...
			ScorableHandMeld::Anjun(t) |
			ScorableHandMeld::Minjun(t) => t.0 as u16,
		};
...
impl ScorableHandFourthMeld {
	...
	///
	/// For the sake of efficiency, this function does not check that the given tiles form a valid ankou.
	pub fn shanpon(t1: Tile, t2: Tile, t3: Tile, tsumo_or_ron: TsumoOrRon) -> Self {
		debug_assert!([t1, t2].eq_ignore_red(&[t2, t3]));
	...
	///
	/// For the sake of efficiency, this function does not check that the given tiles form a valid anjun.
	pub fn kanchan(t1: ShunLowTile, t2: NumberTile, t3: NumberTile, tsumo_or_ron: TsumoOrRon) -> Self {
	...
	///
	/// For the sake of efficiency, this function does not check that the given tiles form a valid anjun.
	pub fn penchan(t1: ShunLowTile, t2: NumberTile, t3: NumberTile, tsumo_or_ron: TsumoOrRon) -> Self {
		let tile = ShunLowTileAndHasFiveRed::new(t1, t2, t3);
	...
	///
	/// For the sake of efficiency, this function does not check that the given tiles form a valid anjun.
	pub fn ryanmen_low(t1: ShunLowTile, t2: NumberTile, t3: NumberTile, tsumo_or_ron: TsumoOrRon) -> Self {
		let tile = ShunLowTileAndHasFiveRed::new(t1, t2, t3);
	...
	///
	/// For the sake of efficiency, this function does not check that the given tiles form a valid anjun.
	pub fn ryanmen_high(t1: ShunLowTile, t2: NumberTile, t3: NumberTile, tsumo_or_ron: TsumoOrRon) -> Self {
		let tile = ShunLowTileAndHasFiveRed::new(t1, t2, t3);
...
impl Ord for ScorableHandFourthMeld {
	fn cmp(&self, other: &Self) -> core::cmp::Ordering {
		...
		const fn sort_criteria(m: ScorableHandFourthMeld) -> u16 {
			let (d, t, tsumo_or_ron) = match m {
				ScorableHandFourthMeld::Tanki(_) => unsafe { core::hint::unreachable_unchecked(); },
				ScorableHandFourthMeld::Shanpon { tile, tsumo_or_ron, .. } => (0, tile as u8, tsumo_or_ron),
				ScorableHandFourthMeld::Kanchan { tile, tsumo_or_ron, .. } => (1, tile.0, tsumo_or_ron),
				ScorableHandFourthMeld::Penchan { tile, tsumo_or_ron, .. } => (2, tile.0, tsumo_or_ron),
				ScorableHandFourthMeld::RyanmenLow { tile, tsumo_or_ron, .. } => (3, tile.0, tsumo_or_ron),
				ScorableHandFourthMeld::RyanmenHigh { tile, tsumo_or_ron, .. } => (4, tile.0, tsumo_or_ron),
			};
			let t = t as u16 & !0b1;
			let dt = d | (t << 3);
...
impl ScorableHandPair {
	...
	/// # Panics
	///
	/// Panics if `t1.ne_ignore_red(&t2)`
	pub fn new(t1: Tile, t2: Tile) -> Self {
		assert!(t1.eq_ignore_red(&t2));
		unsafe { Self::new_unchecked(t1, t2) }
	}
	...
	fn chanta_routou(self) -> ChantaRoutou {
		let terminals = t34set! { 1m, 9m, 1p, 9p, 1s, 9s };
		let honors = t34set! { E, S, W, G, N, Wh, G, R };
