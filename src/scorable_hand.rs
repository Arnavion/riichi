...
#[repr(transparent)]
struct ScorableHandMeldSortCriteria(u16);
...
impl ScorableHandMeldSortCriteria {
	...
	pub(crate) const fn new(m: &ScorableHandMeld) -> Self {
		...
		// So we instead combine them into a `u16` and use that as the comparison key.
	...
	fn cmp(&self, other: &Self) -> core::cmp::Ordering {
		...
		const fn sort_criteria(m: ScorableHandFourthMeld) -> u16 {
			let (d, t, tsumo_or_ron) = match m {
				...
				ScorableHandFourthMeld::Shanpon { tile, tsumo_or_ron, .. } => (0, tile as u16, tsumo_or_ron),
				ScorableHandFourthMeld::Kanchan { tile, tsumo_or_ron, .. } => (1, tile as u16, tsumo_or_ron),
				ScorableHandFourthMeld::Penchan { tile, tsumo_or_ron, .. } => (2, tile as u16, tsumo_or_ron),
				ScorableHandFourthMeld::RyanmenLow { tile, tsumo_or_ron, .. } => (3, tile as u16, tsumo_or_ron),
				ScorableHandFourthMeld::RyanmenHigh { tile, tsumo_or_ron, .. } => (4, tile as u16, tsumo_or_ron),
			};
			let dt = d | (t << 3);
