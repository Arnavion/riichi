	...
	pub(crate) fn num_peikou_isshoku_sanjun(&self) -> (usize, bool) {
		...
		cfg_select! {
			all(target_arch = "riscv64", not(target_feature = "v")) => {
				...
				let mut peikou_is_valid = is_closed;
				...
				let mut isshoku_sanjun_is_valid = is_closed;
				...
				let consider = core::simd::cmp::SimdPartialEq::simd_eq(ds.extract::<0, 3>(), core::simd::Simd::splat(4));
				let offsets = ts & core::simd::Simd::splat(!0b1);
				let count_adjustments = core::simd::Select::select(consider, core::simd::Simd::splat(1), core::simd::Simd::splat(0)) << core::simd::num::SimdUint::cast::<u64>(offsets.extract::<0, 3>());
				...
				{
					let m4_d = ds[3];
					let consider_m4 = m4_d >= 4; // Anjun, or minjun non-tanki (since minjun tanki already caused `peikou_is_valid = is_closed` above to be `false`.)
			...
			_ => {
				let consider = core::simd::cmp::SimdPartialEq::simd_eq(
					ds & core::simd::Simd::from_array([0x07, 0x07, 0x07, 0x04]),
				...
				let mut counts = core::simd::Simd::<u8, 34>::splat(0);
				for i in 0..4 {
				...
				let num_peikou = if is_closed { num_peikou } else { 0 };
				let is_isshoku_sanjun = is_closed & is_isshoku_sanjun;
				(num_peikou, is_isshoku_sanjun)
			},
		}
	}
...
const impl Clone for ScorableHandRegular {
	fn clone(&self) -> Self {
		*self
	}
}
...
const impl From<ScorableHandMeld> for ScorableHandFourthMeld {
