...
pub(crate) fn run(path: &std::path::Path) -> std::io::Result<()> {
	...
	write!(f, "type R=(u32,Option<Honor>,Meld<Honor>);")?;

	write!(f, "pub const ZEROS:&[R]=&[(0,N,M0)];")?;
	...
	for (i, (key, p)) in maps.twos.into_iter().enumerate() {
		...
		write!(f, "({},Y({p}),M0)", SmallInt(key as usize))?;
	}
	write!(f, "];")?;
