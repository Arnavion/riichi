...
pub(crate) fn run(path: &std::path::Path) -> std::io::Result<()> {
	...
	write!(f, "pub const STORAGE:&[(Option<NumberMeld>,Meld<NumberMeld>)]=&[")?;
	...
	let fives: std::io::Result<std::collections::BTreeMap<_, _>> =
		...
		.map(|(key, values)| {
			assert!(values.is_sorted(), "{values:?}");
			assert!(values.array_windows().all(|[m1, m2]| m1 != m2), "{values:?}");
			let storage_start = storage_i;
	...
	let eights: std::io::Result<std::collections::BTreeMap<_, _>> =
		...
		.map(|(key, values)| {
			...
			let storage_len = values.len();
			storage_i += storage_len;
			for (p, [m1, m2]) in values {
				write!(f, ",(Y({p}),m2({m1},{m2}))")?;
			}
			Ok((key, (storage_start, storage_start + storage_len)))
		})
		.collect();
	...
	let elevens: std::io::Result<std::collections::BTreeMap<_, _>> =
		...
		.map(|(key, values)| {
			...
			let storage_len = values.len();
			storage_i += storage_len;
			for (p, [m1, m2, m3]) in values {
				write!(f, ",(Y({p}),m3({m1},{m2},{m3}))")?;
			}
			Ok((key, (storage_start, storage_start + storage_len)))
		})
		.collect();
	...
	let fourteens: std::io::Result<std::collections::BTreeMap<_, _>> =
		...
		.map(|(key, values)| {
			...
			let storage_len = values.len();
			storage_i += storage_len;
			for (p, [m1, m2, m3, m4]) in values {
				write!(f, ",(Y({p}),m4({m1},{m2},{m3},{m4}))")?;
			}
			Ok((key, (storage_start, storage_start + storage_len)))
		})
		.collect();
...
impl Element for Number {
	...
	fn elements_of_meld(m: Self::Meld) -> [Self; 3] {
		match m {
			NumberMeld::K1 => [Number::One; 3],
			NumberMeld::K2 => [Number::Two; 3],
			NumberMeld::K3 => [Number::Three; 3],
			NumberMeld::K4 => [Number::Four; 3],
			NumberMeld::K5 => [Number::Five; 3],
			NumberMeld::K0 => [Number::Five, Number::Five, Number::FiveRed],
			NumberMeld::K6 => [Number::Six; 3],
			NumberMeld::K7 => [Number::Seven; 3],
			NumberMeld::K8 => [Number::Eight; 3],
			NumberMeld::K9 => [Number::Nine; 3],
	...
	fn elements_of_pair(p: Self::Pair) -> [Self; 2] {
		match p {
			NumberMeld::K1 => [Number::One; 2],
			NumberMeld::K2 => [Number::Two; 2],
			NumberMeld::K3 => [Number::Three; 2],
			NumberMeld::K4 => [Number::Four; 2],
			NumberMeld::K5 => [Number::Five; 2],
			NumberMeld::K0 => [Number::Five, Number::FiveRed],
			NumberMeld::K6 => [Number::Six; 2],
			NumberMeld::K7 => [Number::Seven; 2],
			NumberMeld::K8 => [Number::Eight; 2],
			NumberMeld::K9 => [Number::Nine; 2],
			_ => unreachable!(),
		}
	}
